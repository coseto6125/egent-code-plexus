#!/usr/bin/env python3
"""Regression-style benchmark for the cgn CLI — runs every public subcommand
against a sample repo and reports wall-clock latency.

Typical use:
    python scripts/benchmark_cgn.py                              # full sweep
    python scripts/benchmark_cgn.py --runs 5 --json out.json     # CI mode
    python scripts/benchmark_cgn.py --skip-cold                  # don't wipe index
"""
...
def _admin_drop(binary: Path, repo: Path) -> None:
    """Issue `cgn admin drop --repo <repo>` to wipe this repo's index.
...
def main() -> int:
    ...
    if not args.binary.exists():
        print(
            ...
            f"`cargo build --release -p code-graph-nexus --bin cgn`",
            file=sys.stderr,
        )
        return 1
    ...
    if not args.skip_cold:
        print(f"→ cgn admin drop --repo {args.repo}")
