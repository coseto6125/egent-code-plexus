...
# `.sample_repo` corpus + ref-gitnexus / ecp source.
_EQUIV_CLASSES: list[set[str]] = [
    # ref-gitnexus emits TS / JS / Dart constructors as Method (their
    # underlying tree-sitter node is method_definition); ecp promotes
    ...
    # Trait joins this class so Swift `protocol P {}` (ecp emits Trait,
    ...
    # EQUIV pairing fails. Union joins the class because ecp C parser
...
# ecp collapses both into a single `Function` node (the callable view,
# which is what `ecp search "fn"` should resolve to). The leftover ref-
...
# at the enclosing `template_declaration` wrapper. ecp emits only `Class`
...
# inherited from ecp's `RawRoute { method, path }` flattening to a single
