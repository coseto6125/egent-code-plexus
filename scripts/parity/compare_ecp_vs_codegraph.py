...
ECP_DROP_KINDS = {"File", "Folder", "Import", "Module"}
...
def get_codegraph_symbols(db_path: Path):
    ...
    for kind, file_path, name in c.fetchall():
        ...
        # Some normalizations for kinds (codegraph uses "constant", ecp uses "Const")
        if norm_kind == "constant":
            norm_kind = "const"
        if norm_kind == "interface":
            norm_kind = "trait" # ecp maps interface to Trait/Class usually
...
def get_ecp_symbols(repo_path: Path):
    print(f"Reading from ecp index for: {repo_path}")
    cmd = [
        "ecp",
        "cypher",
    ...
    try:
        ...
        print(f"ecp command failed: {e.stderr}")
        sys.exit(1)
    ...
    if "{" in output:
        ...
        print("Could not parse JSON from ecp output")
        sys.exit(1)
        
    symbols = set()
    for row in data.get("rows", []):
        kind, file_path, name = row
        if kind in ECP_DROP_KINDS:
            continue
        
        # In ecp, n.filePath could be null if it's a global/synthetic node
        if not file_path:
            continue
            
        norm_kind = kind.lower()
        if norm_kind == "class":
            # codegraph differentiates struct/class/interface, ecp might just use Class/Trait
            pass 
...
def main():
    ...
    ecp_symbols = get_ecp_symbols(repo_path)
    ...
    ecp_pairs = {(f, n) for k, f, n in ecp_symbols}
    
    common_pairs = cg_pairs.intersection(ecp_pairs)
    cg_only_pairs = cg_pairs - ecp_pairs
    ecp_only_pairs = ecp_pairs - cg_pairs
    
    print("\n--- Symbol Extraction Parity ---")
    print(f"codegraph total symbols: {len(codegraph_symbols)}")
    print(f"ecp total symbols:       {len(ecp_symbols)}")
    print(f"Common symbols (name+file): {len(common_pairs)}")
    print(f"codegraph ONLY:          {len(cg_only_pairs)}")
    print(f"ecp ONLY:                {len(ecp_only_pairs)}")
    
    with open("scripts/parity/cg_only.txt", "w") as f:
        for p in sorted(list(cg_only_pairs)):
            f.write(f"{p[0]}\t{p[1]}\n")
            
    with open("scripts/parity/ecp_only.txt", "w") as f:
        for p in sorted(list(ecp_only_pairs)):
            f.write(f"{p[0]}\t{p[1]}\n")
            
    print("\nWrote detailed diffs to scripts/parity/cg_only.txt and scripts/parity/ecp_only.txt")
