...
def clone_repo(name: str, url: str) -> Path:
    ...
    if not repo_path.exists():
        ...
        subprocess.run(["git", "clone", "--depth", "1", url, str(repo_path)], check=True, capture_output=True)
    return repo_path
...
def benchmark_analyze(repo_path: Path):
    ...
    start_time = time.time()
    subprocess.run(["cargo", "run", "--release", "--bin", "gnx-rs", "--", "analyze", "--repo", str(repo_path)], cwd=workspace, capture_output=True)
...
def main():
    ...
    subprocess.run(["cargo", "build", "--release", "-p", "gnx-cli"], capture_output=True, check=True)
    ...
    for lang in test_subset:
        ...
        try:
            ...
            print(f"    \u2714 gnx: {gnx_time:.2f}s | gnx-rs: {gnx_rs_time:.2f}s | Speedup: {speedup:.1f}x\n")
