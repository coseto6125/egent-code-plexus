...
### 상위 GitNexus 대비 60× 빠른 콜드 인덱스

[gitnexus](https://github.com/abhigyanpatwari/GitNexus) TypeScript 코드베이스에서 측정 · `../../scripts/parity/benchmark_vs_gitnexus.py`:

| 단계 | ecp (Rust) | gitnexus (Node) | 속도 향상 |
|---|---|---|---|
| **콜드 인덱스** | **~970 ms** | ~58 s | **60×** |
| **심볼 컨텍스트** | **~70 ms** | ~430 ms | **6×** |
| **블라스트 반경** | **~70 ms** | ~460 ms | **6×** |
| **Cypher 쿼리** | **~70 ms** | ~400 ms | **5×** |

*`ecp` 지연 시간에는 전체 프로세스 시작 시간이 포함됩니다(데몬 없음). GitNexus (v1.6.5)는 워밍업된 인덱스 저장소 기준으로 측정.*
