...
3 ツール直接対決：[`codegraph`](https://github.com/colbymchenry/codegraph)（Node + SQLite）と上流 [`gitnexus`](https://github.com/abhigyanpatwari/GitNexus)（Node）——同一 checkout、同一マシン。`ecp` はステートレスなワンショット CLI：以下のレイテンシはすべて**完全なプロセス起動を含み**、デーモンなし、ウォームアップなし。

*バージョン：`ecp` 0.4.2 · `codegraph` 0.9.4 · `gitnexus` 1.6.5。設定可能な場合はすべてのツールを 1 MiB の最大ファイルサイズ上限に設定（`gitnexus` は 512 KB ハードコード）。`ecp` は 5–7 回の実行での中央値。ハードウェア：AMD Ryzen 9 9950X（16 論理コア）、Linux。*

### `microsoft/vscode` — 14,874 ファイル、密集した単一言語 TypeScript

| メトリクス | **`ecp`** | `codegraph` | `gitnexus` |
|---|---|---|---|
| **コールドインデックス** | **4.6 s** | 166.9 s | **DNF** — 27 分後に強制終了 |
| ピーク RSS | **~1.0 GiB** | 1.7 GiB | 4.6 GiB（上昇中） |
| シンボル検索 / クエリ | **34.6 ms** | 169.5 ms | — |
| 呼び出し元 / ブラスト半径 | **27.2 ms** | 172.4 ms | — |
| インスペクト / コンテキスト | **35.0 ms** | 415.9 ms | — |
| 影響ベースライン（git-diff） | **725.9 ms** | N/A — このモードなし | — |
| グラフノード数 | **507,257** | 315,498 | — |
| グラフエッジ数 | 916,380 | **986,709** | — |
| ディスク上のインデックスサイズ | **87 MiB** | 671 MiB | — |
| インデックスされたファイル数 | **14,874** | 10,814 | — |

*`gitnexus` は完了しませんでした——インメモリグラフ解決フェーズで 27 分スタックした後に強制終了（RSS 4.6 GiB、出力なし）。*

### `abhigyanpatwari/GitNexus` — 3,232 ファイル、ポリグロット（3 ツールすべてが完了できるコーパス）

| メトリクス | **`ecp`** | `codegraph` | `gitnexus` |
|---|---|---|---|
| **コールドインデックス** | **0.74 s** | 11.2 s | 77.6 s |
| ピーク RSS | **264 MiB** | 501 MiB | 2.5 GiB |
| 検索 / クエリ | **9.4 ms** | 103.5 ms | — |
| 呼び出し元 / ブラスト半径 | **9.2 ms** | 104.2 ms | 297.6 ms |
| インスペクト / コンテキスト | **9.4 ms** | — | 295.5 ms |
| グラフノード数 | **49,122** | 19,604 | 30,223 |
| グラフエッジ数 | **48,271** | 39,155 | 47,218 |
| ディスク上のインデックスサイズ | **7.7 MiB** | 37 MiB | 306 MiB |
| インデックスされたファイル数 | **3,232** | 2,968 | 3,232 |

**コールドインデックス：`codegraph` より 15–37× 高速；`gitnexus` は実際の大規模リポジトリでは完了しない。あらゆるスケールで最小メモリ、最小ディスクインデックス、最密グラフ。**
