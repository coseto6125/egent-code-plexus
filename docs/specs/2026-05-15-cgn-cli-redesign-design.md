# cgn CLI Redesign — Agent-First Surface
...
The current `cgn` CLI exposes 27 top-level subcommands, mixing query/edit/refactor commands (for LLM agents) with registry/setup/destructive operations (for humans). For an LLM consuming `cgn --help`, this surface is noisy: 17 commands the agent rarely or never invokes, ambiguous verbs (`clean` / `remove`), and per-command flags that vary between commands. Empirically, the agent's first read of `--help` does not narrow its toolchain to "what to use right now".
...
| **P7. Admin invisible from agent** | `cgn admin <cmd>` exists for human operators but is `hide = true` at the top level — `cgn --help` does not mention it. |

---

## 3. CLI Surface — Agent Side

`cgn --help` lists exactly **9 commands**:

```
GitNexus query engine for code intelligence.

Usage: cgn [OPTIONS] <COMMAND>
...
**Empty / error (P5)**:
- Symbol not found → `No symbol "X". Did you mean: <fuzzy 3>? Or: cgn search X --mode bm25`
...
#### 3.2.3 `cgn impact <name | --since <ref>>`
...
- `cgn impact validateUser` → blast radius from one symbol
- `cgn impact --since HEAD~1` → blast radius across all symbols changed since `HEAD~1` (collapses the old `detect-changes` / `diff` flow)
...
→ To also rename in markdown: cgn rename validateUser checkUser --markdown
→ To inspect new symbol:      cgn inspect checkUser
```
...
- Parse error → `Cypher parse error at col <N>: <token>` + tip pointing to `cgn cypher --help` for grammar.

#### 3.2.6 `cgn coverage`

Single entry point for "what do I have / how healthy is it". Folds the old `doctor` + `status` + `list` + `overview`. External-client (HTTP/DB/Redis/queue) usage detail is **not** folded here — see the standalone `cgn tool-map` command (its per-callsite binding analysis sits beyond a health summary's granularity).
...
> **Status (2026-05-18, PR #146):** `--repo @<group>` on top-level commands now returns an error pointing at `cgn group coverage <name>`; the noun-first surface is canonical for group-scoped reads. `--repo @all` (registry-wide) is unchanged.

**Empty**: never (always reports framework coverage; if 0 frameworks detected, says so + suggests `cgn admin index --reembed`).

**Multi-repo**: yes.

#### 3.2.7 `cgn routes [<path>]`
...
**Empty (P5)**: `No HTTP routes detected. Frameworks scanned: [list]. Possible: framework not supported / no route declarations / coverage gap (see cgn coverage --blind-spots)`
...
> **Status (2026-05-18, PR #146):** The group-scoped flow moved to `cgn group contracts <name>`; top-level `--repo @<group>` is rejected with a hint pointing there. `--repo @all` (registry-wide) still works on `cgn contracts`.

**Empty (P5)**:
- Group has only 1 repo → `Group "<g>" has 1 member; cross-repo contracts need ≥2 repos`
- No matches in multi-repo selector → `No cross-repo contracts found. Possible: no @route + @client pairings, or framework coverage gap (see cgn coverage)`

---

## 4. CLI Surface — Admin Side

Admin is a **nested subcommand**, hidden from the top-level `cgn --help`:

```rust
Commands::Admin(AdminCommands)  // #[command(hide = true)]
```

`cgn admin --help` lists:

```
GitNexus administrative operations (registry, hooks, destructive ops).

Usage: cgn admin <COMMAND>
...
| `verify-resolver` | cgn-developer QA tool (resolver dump vs language oracle) |
...
2. For each repo:
   ...
   b. If missing:
      ...
      - Run index synchronously (same code path as `cgn admin index`).
   ...
   c. If present but stale (mtime < newest source file):
      ...
      - Agent / hook may proactively run `cgn admin index` after large changes.
```
...
**Resolution failure**: `--repo "x,y"` where `y` doesn't resolve → fail with itemized list of which atoms didn't resolve + suggestion (`cgn list` or `cgn coverage`).
...
✗ Index build failed for "/path/x":
  ...
  next:  cgn coverage --repo /path/x --blind-spots
```
...
- `cgn admin group add <repo> <group>` — appends `<group>` to `RepoEntry.groups` (if not already present); auto-creates `GroupEntry` if needed; adds `<repo>` to `GroupEntry.members`.
- `cgn admin group remove <repo> <group>` — removes `<group>` from `RepoEntry.groups` and `<repo>` from `GroupEntry.members`; if `GroupEntry.members` becomes empty, deletes the `GroupEntry`.
...
- `CLAUDE.md` (cgn workflow section in user's global instructions)
- Any test scripts in `crates/cgn-cli/tests/` that invoke commands by name
- Any references in `scripts/parity/` and `scripts/benchmark_cgn.py`
...
| Admin (under `cgn admin`) | (mixed) | **7 entries at `cgn admin --help`** (6 leaves + 1 `group` parent), with 2 nested subcommands under `cgn admin group` |
| Hidden internal | 2 | **3** |
...
Added: `contracts`, nested `admin group add/remove`, multi-group `RepoEntry.groups`, auto-ensure indexing, server-side rename verification, hybrid search modes. (`scan` was added in this redesign and later removed in the 2026-05-17 review-aggregator pivot — see `docs/feat/2026-05-17-cgn-review-spec.md`.)
...
- **CLI surface tests**: integration tests verify `cgn --help` exposes exactly the 9 agent commands; `cgn admin --help` exposes exactly the 7 admin commands; hidden commands are runnable but absent from `--help` output.
...
Existing test infrastructure under `crates/cgn-cli/tests/` is the home base.
...
- **`cgn update`** / self-update CLI — out of CLI redesign scope.
...
9. User insight: `index` keeps an agent face via auto-ensure; explicit `cgn admin index` remains for human/bulk operations.
