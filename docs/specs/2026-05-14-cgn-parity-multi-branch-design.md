# code-graph-nexus 對齊 + 多 branch + AST rename 設計
...
衍生後 canonicalize 完整 path，assert `starts_with(~/.cgn/)`。失敗 fallback 走 `<working-tree basename>`，再撞名加 hash。

### 1.2 `<branch>` sanitization

`feat/foo-bar` → `feat__foo-bar`；`/` → `__`；其他不合法字元 → `_`。
與現有 wrapper（`cgn.branch-spike`）行為一致，方便共存遷移。
...
- **Magic byte**：`CGN-RS\0\0`（cosmetic + version sentinel）
- **Version field**：u32 schema version — 不相容自動拒絕載入並提示 `cgn admin index --force`

```
graph.bin layout:
+0   8 byte : magic "CGN-RS\0\0"
...
- `cgn inspect X` (who calls X) 是 LLM review 的 hot path — 一個 session 50-100 次
...
- `file_node_offsets`：`cgn inspect src/auth.ts` → O(log N) file 找 idx + O(K) 取 K 個節點，避免遍歷全圖
- `kind_offsets` + `kind_data`：`cgn find --kind Method` → O(K) 直接拿 K 個 Method 節點
...
```
cgn impact --since HEAD~1 → 列出變更檔
cgn admin index（增量模式） <file>:
...
定義位置：`cgn-core::graph::merged_view::MergedView<'a>`，內含 `&'a Archived<ZeroCopyGraph>` (base) + `&'a Archived<DeltaGraph>` (delta) 兩個 reference。
...
#### 實測 baseline（2026-05-14 measure，code-graph-nexus `target/release/cgn` v0.1.0）
...
|---|---:|---:|---:|---:|---:|
| cgn-core (Rust)       | ~10k     | 119   | 294 ms | **68 MB**  | 18 KB |
...
|---|---:|---:|
| `cgn inspect main` | **28 MB** | < 10 ms |
| `cgn search --query main`  | **29 MB** | < 10 ms |
| `cgn impact main`| **12 MB** | < 10 ms |
...
`cgn-core::registry::archive_cache` 提供 **per-process OnceLock**，把驗過的 `&Archived<ZeroCopyGraph>` 快取在 mmap fd 旁邊。下個 query 不重複驗。LLM session 跑 100 個 `cgn context` 從 ~1.5 s（每次驗）變成 ~10 ms（共用）。

cache 自身 footprint：50 byte（pointer + OnceLock header），相對 RSS 可忽略。

#### 可選硬上限（opt-in）

env `CGN_RAM_LIMIT_MB=N` → 啟動時 `setrlimit(RLIMIT_AS, N×MB)`（Linux）：
...
**OCI 2 vCPU + 4 GB RAM 推薦**：`CGN_RAM_LIMIT_MB=2048 --skip-embeddings true`，覆蓋至 ~1.5M LoC 級 repo（基於 1.2 MB/kLoC 線性外推）
...
|---|---|---|---|
| **I1** | analyze 進程 RSS 受控（軟上限 500 MB warn，硬上限 `CGN_RAM_LIMIT_MB` 或無）| OS OOM killer 殺進程 | C9 StringPool watermark + C10 Delta cutoff + H5 setrlimit catch ENOMEM |
...
- I2: 1 GB graph.bin + `CGN_RAM_LIMIT_MB=128` 跑 query → RSS < 128 MB 且不 fail（mmap 不算進去）
...
- env `CGN_MAX_FILE_SIZE_KB` 或 `GITNEXUS_MAX_FILE_SIZE`（同名兼容）覆寫
...
統一函式置於 `cgn-core::registry::path::uid_path()`，分析器與 rename 共用，避免 drift。
...
- parse 失敗自動嘗試 `.bak`，仍失敗則 rebuild from disk（walk `~/.cgn/*/*/meta.json`）。每個 `meta.json` 含 `worktree_path` / `remote_url` / `schema_version`，足以還原完整 registry entry（含 cross-repo / group 資訊另從 audit log 重建）

**Layer 2 — graph.bin per branch**（local，每個 `<branch>/` 獨立）：
- 同一 `<repo>/<branch>/` 下 `cgn admin index` 並發會撞 graph.bin
...
sequenceDiagram
    participant A as cgn admin index (worker A)
    participant B as cgn admin rename-branch (worker B)
...
├── cgn-analyzer/         （既有 + queries.scm 增 usage_identifier）
└── cgn-cli/src/
    ├── commands/
...
### 4.1 安裝（`cgn admin install-hook`）

`cgn admin install-hook` 步驟：
1. 解析 `git rev-parse --git-common-dir`
2. `which cgn` 解析絕對路徑 P
3. 寫 hook：
   ```sh
   #!/bin/sh
   # cgn-managed reference-transaction hook
   exec /home/enor/.cargo/bin/cgn hook-handle "$@"
   ```
4. `chmod 0755 <hook>`
5. 若 hook 已存在且非 cgn-managed → 印警告並備份成 `<hook>.bak.<timestamp>`，不強制覆寫，需 `--force`
6. shared-host 偵測（H4 警告）

### 4.2 處理（`cgn hook-handle`）
...
4. 對每個 deleted branch：
   ...
   - child 主流程：
     ...
     - 命中 → 呼 `cgn admin rename-branch --from=<deleted> --to=<current> --repo=<repo>`
     - 未命中 → 呼 `cgn admin prune --branch=<deleted> --repo=<repo>`
...
flowchart TD
    ...
    SC --> M{match found?}
    M -->|yes| REN[cgn admin rename-branch<br/>--from=X --to=Y]
    M -->|no| PRU[cgn admin prune --branch=X]
...
`fork()` + `setsid()` 是 POSIX-only。跨平台抽象成 `cgn_core::daemon::spawn_detached()`：
...
|---|---|---|---|
| Linux / macOS | `#!/bin/sh\nexec /abs/path/cgn hook-handle "$@"` | `fork() + setsid()` | `git reflog` POSIX |
...
1. **`cgn admin install-hook` 偵測 Git 環境**：必須有 Git for Windows（內含 `sh.exe`）；無 → warn `please install Git for Windows ≥ 2.30 for hook support`
...
flowchart LR
    ...
    subgraph S2["Stage 2: Verify (tree-sitter, rayon parallel)"]
        ...
        PARSE --> Q["query @cgn.usage<br/>filter text == target_name"]
    ...
    subgraph S3["Stage 3: Execute (atomic per file)"]
        ...
        R --> W[write .cgn.tmp + fsync<br/>rename atomic]
        W --> OK[success]
        W --> FAIL[failure → rollback .cgn.bak]
    end
...
- target symbol 透過 `cgn inspect` 同款邏輯解析 UID
...
- 跑 `usage_identifier` query（每語言 queries.scm 加 capture group）：
  ```scheme
  ;; e.g. rust
  (call_expression function: (identifier) @cgn.usage)
  (generic_type type: (type_identifier) @cgn.usage)
  ```
...
- 寫 `path.cgn.tmp` → fsync → `rename()` atomic
- 任一 file 失敗 → rollback 所有已寫檔（先 backup .cgn.bak）
...
```
$ cgn rename --symbol oldName --new_name newName --dry_run=true
...
14 種語言每個加 `@cgn.usage` capture group。預估每語言 15-30 min（多數已有 identifier 抓取，只是分 def vs usage）。
...
flowchart LR
    ...
    subgraph IDX["Index (~/.cgn/&lt;repo&gt;/&lt;branch&gt;/)"]
        AST["tree-sitter AST"]
...
```bash
cgn find --orphans --kind Method --limit 50
cgn find --god-classes --threshold 20
cgn find --circular-imports
cgn find --accesses Property:password
cgn find --name-prefix login --include-body --kind Method
```
...
| ID | 風險 | Mitigation |
|---|---|---|
| C1 | path traversal in `<repo>` | regex whitelist + canonicalize assert under `~/.cgn/` |
...
| C8 | delta.bin 損壞讓 merge query 噴錯 / 假結果 | delta 也走 rkyv bytecheck；parse 失敗自動降級為 base-only view 並警示；提供 `cgn sync --rebuild-delta` 重建（從 git status 推 patch list 重跑 analyze --patch）|
...
| H2 | ~/.cgn/ permission leak | chmod 0700 on create + verify on each write |
...
| H5 | 巨型 monorepo analyze 觸發 OOM kill | 可選 `CGN_RAM_LIMIT_MB` env → `setrlimit(RLIMIT_AS)`；malloc 失敗 → catch + 釋放 working 緩衝 + graceful exit；audit log 記 `oom.aborted` |
...
| H7 | 既有 non-cgn hook（git-lfs / lefthook）被覆寫破壞 | `cgn init` 預設 hook chaining：偵測既有 hook → 在 shim 內先 `exec` 舊 hook 再呼叫 `cgn hook-handle`；`--no-chain` flag 才走純備份模式 |
...
| Per-branch graph.bin lock | integration: 並發跑兩個 `cgn admin index` 同 branch，後者乾淨退出（無 corruption） |
...
| **setrlimit graceful** | integration (Linux): `CGN_RAM_LIMIT_MB=64 cgn admin index` 大目錄 → 應 graceful exit 而非 abort/segfault；audit 記 `oom.aborted` |
| **mmap query 免疫** | integration: 1GB graph.bin + `CGN_RAM_LIMIT_MB=128` 跑 `cgn inspect` → 成功（mmap demand-load 不算 RSS）|
...
| **Registry double-corrupt rebuild** | integration: 刪除 registry.json + .bak → `cgn coverage` 從 meta.json 重建 → 列表正確含 worktree_path / remote_url |
| **Delta schema drift** | unit: base schema_version=2, delta base_schema_version=1 → `MergedView::open` 自動降級為 base-only + warn |
| **Hook chaining** | integration: 預先放一個 dummy reference-transaction hook → `cgn admin install-hook` → 新 shim 應該先 exec 舊 hook 再 exec cgn hook-handle |
| **Windows hook spawn_detached** | integration on `windows-latest`: `cgn admin install-hook` → `git branch -m a b` → 等 500ms → registry 已更新；驗 child process 用 `DETACHED_PROCESS` flag 啟動且 parent 立即 return |
| **Path normalization (Windows)** | unit: `C:\Users\enor\repo` → hook shim 內寫成 `/c/Users/enor/repo` 形式 sh.exe 可解 |
| **Group sync offline member** | integration: group 含 worktree_path 不存在的 member → `cgn group_sync` partial success，failed member 寫 audit，其他成員仍完成 |
...
```
主線執行緒：
  [1] cgn-core::registry/ + audit.rs + path.rs + lock.rs       3d
  [2] cgn-cli::git::safe_exec.rs（hardening 封裝）              0.5d
...
| 決定 | 取捨 |
|---|---|
| 中央化 storage `~/.cgn/` | 易管理 + 跨 repo 統一；代價：所有 repo 共用 disk quota，shared-host 需小心權限 |
...
| 無 MCP server daemon | 純 CLI 簡化部署；代價：失去 daemon 化 IPC 加速（code-graph-nexus 啟動快，影響小） |
...
| User override `CGN_MAX_FILE_SIZE_KB=32768` + `--include-vendor` | H3 parse budget + ts_set_allocator |
