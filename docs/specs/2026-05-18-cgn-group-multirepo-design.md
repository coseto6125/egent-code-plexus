# `cgn group` — Multi-repo Query Design
...
> **Amendment (2026-05-18, post-merge):** `cgn group search` was folded into
> `cgn group find` with `--merge none|rrf` + `--limit N` + `--batch`. The
> `search` verb no longer exists at the `group` level. Sections below that
> mention `cgn group search` should be read as `cgn group find --merge rrf`
> (RRF-merged top-K) or `cgn group find --merge none` (per-repo concat).
> Rationale: `find` is the noun-first verb at top-level (`cgn find`) and the
...
`cgn` already has the **selector layer** for groups (`@<group>` / `@all` in
...
transfer that knowledge to `cgn` today.

This spec brings the upstream workflow into `cgn` while keeping cgn's
...
|---|---|---|
| `RegistryFile` + `GroupEntry` schema | `cgn-core/src/registry/store.rs` | Source of truth for group membership — **no new YAML/JSON file** |
| `Registry::resolve` | `cgn-core/src/registry/` | Member identifier → graph path. Reused for `group impact --repo <member>` (resolves the same way as `--repo <name>` elsewhere) |
| `ZeroCopyGraph` mmap | `cgn-core` | Per-repo graph reads in extractors and impact fan-out |
| `TantivyEngine` BM25 index | `cgn-cli/src/search.rs` (used via `tantivy_hits` in `commands/find.rs:598`) | Cross-repo contract-id matching in the BM25 stage. Group sync builds a per-group Tantivy index over `contract_id`s and queries it for unmatched contracts — **no new BM25 dependency** |
| `bm25_hits_from_graph` + `substring_hits` fallback | `commands/find.rs` | Pattern same as `cgn find` — Tantivy when index exists, substring scan when missing |
| `atomic_write_json` | `cgn-core/src/registry/io.rs` | `meta.json` writes |
| rkyv `Archive`/`Serialize` derive | already in `cgn-core` deps | `contracts.rkyv` |
| `Config` (`~/.cgn/config.toml`) | `cgn-core/src/config.rs:13` | Adds a `group` section instead of hardcoding thresholds (see "Configuration" below) |
| `commands/admin/config.rs` editor | `commands/admin/config.rs` | New group fields surface in `cgn admin config` without new UI |
| Existing `search` / `find` / `coverage` engines | `commands/{search,find,coverage}.rs` | `cgn group <verb>` are **thin parallel fan-outs**, not re-implementations |
| Existing `impact` engine | `commands/impact.rs` | Local phase of `cgn group impact` — cross fan-out only adds a wrapper |
...
- **Not** a `group.yaml` config file — cgn keeps the source of truth in
  `~/.cgn/registry.json` (`GroupEntry { name, members }`).
...
### Decision: noun-first under `cgn group`, management stays in `admin`

| Category | Command | Status |
|---|---|---|
| Management (writes registry) | `cgn admin group add / remove / list` | Existing — keep as-is |
| Cross-repo queries (new namespace) | `cgn group sync / status / impact / contracts / search / find / coverage` | **New top-level namespace** |
| Single-repo / all-repo queries | `cgn search / find / contracts / coverage --repo <name>|@all` | Existing — keep as-is |

**Removed (breaking)**: `--repo @<group>` on `search / find / contracts /
coverage`. Resolving `@<group>` from these commands returns an error with a
hint pointing to the `cgn group <verb>` equivalent. cgn is pre-1.0; the
callers are scripts and the MCP layer, both of which we control.

Rationale:
- `admin group add/remove` mutates `~/.cgn/registry.json` and belongs with
...
```
cgn admin group add <group> <repo>
cgn admin group remove <group> <repo>
cgn admin group list [name]

cgn group status <name>             # staleness audit (index + contracts)
cgn group sync <name> [flags]       # extract contracts + build cross-links
cgn group contracts <name> [flags]  # inspect contract registry
cgn group impact <name> --target <symbol> --repo <member> [flags]
cgn group search <name> <query> [flags]
cgn group find <name> <pattern> [flags]
cgn group coverage <name> [flags]
```
...
human-readable, and read on every `cgn group status` call without needing to
mmap the contracts archive. `contracts.rkyv` is the hot path (impact /
contracts queries) and stays zero-copy.

**Why rkyv, not SQLite**: upstream's `bridge.lbug` is SQLite. cgn already has
...
### Types (Rust, in new crate `code-graph-nexus-group` or under
`cgn-cli::group::types`)
...
`cgn-core/src/config.rs`), not as `const` literals in group code.
...
pub struct GroupConfig {
    ...
    /// Local impact wall-clock budget for `cgn group impact`.
...
`/metrics`; users add them once via `cgn admin config` if needed.
...
### `cgn group impact <name> --target <symbol> --repo <member>`

Two-phase:

1. **Local impact** — run existing `cgn impact` engine on the named member
...
(`grep -l "@group" crates/cgn-cli/src/commands/`):
...
2. The selector path (`repo_selector.rs::Atom::Group`) stays — it's
   reused by `cgn group <verb>` internals to expand the member list.
   What changes is the **dispatch site**: top-level commands now refuse
   `Atom::Group` before resolution, returning
   `SelectorError::GroupAtTopLevel { hint: "use `cgn group <verb>` instead" }`.
3. `Atom::All` (`@all`) stays usable on top-level commands — it's not a
   group, just every indexed repo, and has no `cgn group` equivalent.

Concretely per command:

| Command | Before | After |
|---|---|---|
| `cgn search --repo @hr` | iterates members serially | error → `cgn group search hr` |
| `cgn contracts --repo @hr` | iterates and concatenates | error → `cgn group contracts hr` |
| `cgn find --repo @hr` | iterates | error → `cgn group find hr` |
| `cgn coverage --repo @hr` | iterates | error → `cgn group coverage hr` |
| `cgn search --repo @all` | unchanged | unchanged |
| `cgn contracts --repo <single>` | unchanged | unchanged |
...
**Today**'s `cgn contracts --repo @hr` and `cgn search --repo @hr`
behavior:
...
**After this change**:
- `cgn group contracts` returns the same raw contract list **plus** an
  ...
  *Precision floor*: every contract the old command listed remains
  listed.
- `cgn group search` defaults to RRF-merged top-K but `--no-merge`
...
`match_type: bm25` so consumers can filter them out if needed — cgn's
...
- `group sync` (first cut, exact + BM25, HTTP+gRPC only): bounded by
  ...
  `scripts/benchmark_cgn.py` extension before merge.
...
a `crates/code-graph-nexus-group/tests/<lang>_<extractor>.rs` fixture".
...
Group-level tests live in `crates/cgn-cli/tests/group_*.rs` and
cover:
...
(a) New crate `crates/code-graph-nexus-group/` containing types + extractors +
    sync engine. CLI calls into it.

(b) Module `cgn_cli::group` under existing CLI crate, with types in
    `cgn_cli::group::types`. Extractors as sibling modules.
...
0. **This spec** committed as `docs(spec): cgn group multi-repo design`.
...
2. `commands/group/extractors/mod.rs` + first HTTP extractor (Go, axum-ish).
   Unit tests in `crates/cgn-cli/tests/group_extract_go_http.rs`.
...
6. `commands/group/sync.rs` + `cgn group sync` wiring. End-to-end test
...
11. **Breaking-change commit**: remove `@group` from top-level
    ...
    `cgn group <verb>`. Update CLI tests + MCP layer (`mcp__cgn_*`
...
12. Doc updates: `docs/skills/cgn.md` mentions the new namespace; README
    Language Matrix gains a "group extractor" column (Wave-1 langs only).

Each commit must pass `cargo test -p code-graph-nexus --tests` and
`cargo clippy -p code-graph-nexus --tests`. Format touched files only with
...
1. **Member identifier for `group impact --repo <member>`** — **uses
   ...
   yaml-key artifact cgn doesn't need; existing `Registry` already
...
3. **MCP layer** — **in scope** (implementation step 11). All MCP
   ...
   CLI. New MCP equivalents (`mcp__cgn_group_sync`,
   `mcp__cgn_group_impact`, etc.) are *deferred to a follow-up PR* —
