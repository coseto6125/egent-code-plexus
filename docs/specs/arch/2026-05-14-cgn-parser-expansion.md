# code-graph-nexus Parser Expansion — Fill gaps + add new languages
...
-   **No shared mutable state during fan-out**: each subagent works on one `crates/cgn-analyzer/src/<lang>/queries.scm` and nothing else. The Config wirings are batched in a separate serial phase (§3.3 Phase 2) so they all touch `config_detector` cleanly.
...
4.  **Verification recipe**:
    ...
    cargo test -p cgn-analyzer --test ast_test   # or per-lang fixture
    cgn admin index --repo tests/parity/fixtures/<lang>/sample_project
    cgn inspect tests/parity/fixtures/<lang>/sample_project:<known_symbol>
    ```
...
```toml
# crates/cgn-analyzer/Cargo.toml
...
Two sites in `crates/cgn-cli/src/commands/analyze.rs` must be updated together:
...
pipeline.register_provider(Box::new(
    cgn_analyzer::lua::parser::LuaProvider::new()?
));
```
...
Also: `crates/cgn-analyzer/src/lib.rs` needs `pub mod lua;`.
...
-   **LSP integration** — out of scope; code-graph-nexus is an indexer, not a language server.
