...
1. 哪邊 emit 了什麼 (`ecp emits: Class` / `ref-gitnexus emits: —`)
...
`real_rs`：ecp 有 emit、ref-gitnexus 同 `(path, name)` 沒有任何 EQUIV
class kind。即「ecp over-emit 或 ref-gitnexus under-emit」。**收進
packet（default）**。

`real_ref`：相反方向。即「ref 有、ecp 沒抓到」— 通常是 ecp parser
...
兩邊都收進來。real_rs 通常是「ecp 太敏感、額外 emit 了 ref 沒抓的東
...
| `bucket · ecp X vs ref Y` | diff 摘要一行打完（不一致才會出現在 packet，所以兩邊 kind 一定有差） |
...
| module-level 但被 arrow-fn 賦值的 const (`const X = () => {…}`) | `design`（ecp 故意 emit 成 Function 而非 Const） |
