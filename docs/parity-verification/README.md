...
跨實作（cgn ↔ ref-gitnexus）的 symbol emission 比對流程。本文件描述
...
`.sample_repo` 有 14 種語言混合 source，是 cgn 和 ref-gitnexus 之間
...
- cgn：用 cypher query 走 `~/.cgn/<repo>__*` 索引
- ref-gitnexus：用其 Python API 走 `.sample_repo/<Lang>/...`

工具：`scripts/parity/dump_per_lang_symbols.py`

dump 完落地成兩個 set：
- `symbol_diffs/<Lang>_rs_only.txt`  ← cgn 有、ref 沒有
- `symbol_diffs/<Lang>_ref_only.txt` ← ref 有、cgn 沒有
...
- 一邊獨有 + kind 在 model-only set（如 rs 的 `EntryPoint` 是 cgn
  獨有 NodeKind）→ `model`
...
3. 對照 cgn parser 設計：是 design choice drop 還是 parser gap

判定規則（**這條規則本身要寫死，不准每輪換**）：

| 樣本性質 | 判定 |
|---|---|
| function/method body 內的 transient declaration（local var、inner fn） | **ref over-emit, cgn design choice** |
| module-level 但被 arrow-fn 賦值的 const (`const X = () => {...}`) | **ref over-emit**（cgn 把它 emit 成 Function 而非 Const） |
| type-level permanent 命名實體（struct field、enum variant field、impl method） | **若 cgn 沒 emit 則為真 cgn bug，要修 parser** |
...
中途有一個是 cgn bug → **立即停下、進入修補階段、reset round 計數**。
...
# - 出現 type-level permanent → cgn bug → 進入修補
...
185 個 Property 是 ref 邊獨有、cgn 邊在同 `(path, name)` 沒任何
equiv class kind。
...
關鍵問題：這算 design choice 還是 cgn bug？
...
「cgn 比 ref 多 17000 個 symbol」不等於「cgn 找到更多 symbol」。
...
cgn 有 `parse_cache`（fingerprint = `"v" + CARGO_PKG_VERSION + "+schema1"`），
...
中途若發現 cgn bug，**reset round 計數**。連續 10 輪 ref-bug 才能宣
...
| 類別 | 證據 | 動作 |
|---|---|---|
| **真 bug** | type-level permanent symbol，cgn 在同 (path, name) 完全沒對應 row | 修 parser + 寫 regression test + 重驗 |
