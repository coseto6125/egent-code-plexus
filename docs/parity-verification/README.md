...
跨實作（ecp ↔ ref-gitnexus）的 symbol emission 比對流程。本文件描述
...
`.sample_repo` 有 14 種語言混合 source，是 ecp 和 ref-gitnexus 之間
...
- ecp：用 cypher query 走 `~/.ecp/<repo>__*` 索引
- ref-gitnexus：用其 Python API 走 `.sample_repo/<Lang>/...`

工具：`scripts/parity/dump_per_lang_symbols.py`

dump 完落地成兩個 set：
- `symbol_diffs/<Lang>_rs_only.txt`  ← ecp 有、ref 沒有
- `symbol_diffs/<Lang>_ref_only.txt` ← ref 有、ecp 沒有
...
- 一邊獨有 + kind 在 model-only set（如 rs 的 `EntryPoint` 是 ecp
  獨有 NodeKind）→ `model`
...
3. 對照 ecp parser 設計：是 design choice drop 還是 parser gap

判定規則（**這條規則本身要寫死，不准每輪換**）：

| 樣本性質 | 判定 |
|---|---|
| function/method body 內的 transient declaration（local var、inner fn） | **ref over-emit, ecp design choice** |
| module-level 但被 arrow-fn 賦值的 const (`const X = () => {...}`) | **ref over-emit**（ecp 把它 emit 成 Function 而非 Const） |
| type-level permanent 命名實體（struct field、enum variant field、impl method） | **若 ecp 沒 emit 則為真 ecp bug，要修 parser** |
...
中途有一個是 ecp bug → **立即停下、進入修補階段、reset round 計數**。
...
ecp admin index --repo /home/enor/egent-code-plexus/.sample_repo --force
# (ref-gitnexus 那邊有自己的 indexer，按 ref repo 文件)

# Step 2: dump 兩邊 symbol 集合
cd /path/to/ecp/worktree
...
# - 出現 type-level permanent → ecp bug → 進入修補

# Step 7（若有 bug）: 修 parser 並寫 regression test
$EDITOR crates/ecp-analyzer/src/<lang>/queries.scm
$EDITOR crates/ecp-analyzer/tests/<lang>_<dimension>.rs
cargo test -p ecp-analyzer --test <lang>_<dimension>

# Step 8: 重 index + 重 dump + 重 aggregate 確認 real_ref 下降
ecp admin index --repo /path/.sample_repo --force
...
185 個 Property 是 ref 邊獨有、ecp 邊在同 `(path, name)` 沒任何
equiv class kind。
...
關鍵問題：這算 design choice 還是 ecp bug？
...
`crates/ecp-analyzer/src/rust/queries.scm`：
...
`crates/ecp-analyzer/tests/rust_enum_variant_fields.rs`：
...
- `cargo test -p ecp-analyzer` → 245+6 tests pass，無 regression
...
「ecp 比 ref 多 17000 個 symbol」不等於「ecp 找到更多 symbol」。
...
ecp 有 `parse_cache`（fingerprint = `"v" + CARGO_PKG_VERSION + "+schema1"`），
重 build binary 後 cache 仍指 old behavior。修 parser 後必 reindex with
`--force`：

```bash
ecp admin index --repo /path --force
```

PR #132 修了 `--force` 不清 parse_cache 的 bug；舊版本可能要手動
`rm -rf ~/.ecp/<repo>__*/parse_cache`。
...
中途若發現 ecp bug，**reset round 計數**。連續 10 輪 ref-bug 才能宣
...
| 類別 | 證據 | 動作 |
|---|---|---|
| **真 bug** | type-level permanent symbol，ecp 在同 (path, name) 完全沒對應 row | 修 parser + 寫 regression test + 重驗 |
