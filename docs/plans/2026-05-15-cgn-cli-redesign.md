# cgn CLI Redesign — Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Rebuild cgn CLI surface from 25 visible commands down to 9 agent commands + 7 admin commands, with multi-group repo membership, auto-ensure indexing, server-side composition (no agent chaining), and a strict empty-result / error / hint output contract. Spec: `docs/specs/2026-05-15-cgn-cli-redesign-design.md`.
...
**Spec reference:** `docs/specs/2026-05-15-cgn-cli-redesign-design.md`
...
```rust
let path = cgn_core::registry::default_path();  // resolves ~/.cgn/registry.json
let reg  = cgn_core::registry::RegistryFile::read_or_empty(&path)
              .map_err(|e| CgnError::Io(format!("registry read: {e}")))?;
// ... mutate reg ...
cgn_core::registry::RegistryFile::write_atomic(&path, &reg)
    .map_err(|e| CgnError::Io(format!("registry write: {e}")))?;
```

If a helper named `registry::load()` doesn't exist when implementing a task, expand to the two-line form above. The naming may evolve; **check `crates/cgn-core/src/registry/mod.rs`** for the canonical API before each task.

Similarly, where the plan writes `Engine::load(&path)` — that's the real API, defined in `crates/cgn-cli/src/engine.rs`.

For `CgnError` variants used (`Output`, `Io`, `Rkyv`): see `crates/cgn-core/src/error.rs` for the actual enum; use the closest fit if a variant name doesn't match.

---

## Pre-flight

This worktree (`worktree-cgn-cli-eval`) was created from origin/main at `95d4228` (before `9bdc8e9 multi_query` was merged). Pull latest before starting:
...
**Create:**
- `crates/cgn-cli/src/commands/inspect.rs` — renamed from `context.rs`, composed output
- `crates/cgn-cli/src/commands/search.rs` — renamed from `query.rs`, hybrid modes + cross-repo fan-out (absorbs multi_query logic)
- `crates/cgn-cli/src/commands/coverage.rs` — folds doctor + status + list + summarize + tool_map summary
- `crates/cgn-cli/src/commands/routes.rs` — folds route_map + api_impact
- `crates/cgn-cli/src/commands/scan.rs` — file-level hallucination check (new)
- `crates/cgn-cli/src/commands/contracts.rs` — cross-repo contracts inventory (new)
- `crates/cgn-cli/src/commands/admin/mod.rs` — admin subcommand namespace
- `crates/cgn-cli/src/commands/admin/group.rs` — `cgn admin group add/remove`
- `crates/cgn-cli/src/repo_selector.rs` — `--repo` selector parser + resolver
- `crates/cgn-cli/src/auto_ensure.rs` — auto-ensure index helper
- `crates/cgn-cli/src/hint.rs` — output contract helpers (empty / error / hint formatters)
- `crates/cgn-cli/tests/inspect_cmd.rs` — replaces `context_cmd.rs`
- `crates/cgn-cli/tests/search_cmd.rs` — replaces `query_cmd.rs`; absorbs `multi_query_cmd.rs`
- `crates/cgn-cli/tests/coverage_cmd.rs` — replaces `doctor_cmd.rs`
- `crates/cgn-cli/tests/routes_cmd.rs` — replaces `api_impact_cmd.rs`
- `crates/cgn-cli/tests/scan_cmd.rs` — new
- `crates/cgn-cli/tests/contracts_cmd.rs` — new
- `crates/cgn-cli/tests/admin_group_cmd.rs` — new
- `crates/cgn-cli/tests/repo_selector.rs` — new
- `crates/cgn-cli/tests/auto_ensure.rs` — new

**Modify:**
- `crates/cgn-cli/src/main.rs` — top-level Commands enum reshape
- `crates/cgn-cli/src/commands/mod.rs` — module list reshape
- `crates/cgn-cli/src/commands/impact.rs` — drop UID-only, add `--name` + `--since` modes
- `crates/cgn-cli/src/commands/rename.rs` — add `--markdown` flag + post-rename verification + collision detection
- `crates/cgn-cli/src/commands/cypher.rs` — error on multi-repo selector
- `crates/cgn-core/src/registry/store.rs` — `RepoEntry.group: Option<String>` → `groups: Vec<String>` + auto-migration
- `crates/cgn-cli/Cargo.toml` — confirm `rayon` is already in deps (added by multi_query)
- `README.md` — update CLI reference section
- `README_zh-TW.md` — update CLI reference section

**Delete (after migration):**
- `crates/cgn-cli/src/commands/context.rs` (→ inspect)
- `crates/cgn-cli/src/commands/query.rs` (→ search)
- `crates/cgn-cli/src/commands/multi_query.rs` (folded into search)
- `crates/cgn-cli/src/commands/doctor.rs` (→ coverage)
- `crates/cgn-cli/src/commands/status.rs` (→ coverage)
- `crates/cgn-cli/src/commands/list.rs` (→ coverage)
- `crates/cgn-cli/src/commands/summarize.rs` (→ coverage)
- `crates/cgn-cli/src/commands/tool_map.rs` (→ coverage --externals)
- `crates/cgn-cli/src/commands/route_map.rs` (→ routes)
- `crates/cgn-cli/src/commands/api_impact.rs` (→ routes)
- `crates/cgn-cli/src/commands/detect_changes.rs` (→ impact --since)
- `crates/cgn-cli/src/commands/cluster.rs` (dropped, use cypher)
- `crates/cgn-cli/src/commands/process.rs` (dropped, use cypher)
- `crates/cgn-cli/src/commands/analyze.rs` (→ admin index — but verify if c08b3e9 already moved it)
- `crates/cgn-cli/src/commands/analyze_here.rs` (→ auto-ensure)
- `crates/cgn-cli/src/commands/index.rs` (recovery; dropped — `admin index` replaces)
- `crates/cgn-cli/src/commands/remove.rs` (folded into admin drop)
...
```bash
cargo build --release --bin cgn 2>&1 | tail -3
./target/release/cgn --help 2>&1 | head -5
```

Expected: build succeeds, `cgn --help` lists current 27 commands (including `multi_query`, `shape_check`).
...
- Create: `crates/cgn-core/src/registry/migrate.rs`
- Test: `crates/cgn-core/src/registry/store.rs` (inline tests at bottom)

- [ ] **Step 1: Write failing migration test**

Add at the bottom of `crates/cgn-core/src/registry/store.rs`:
...
```bash
cargo test -p cgn-core registry::store::migration_tests 2>&1 | tail -15
```
...
In `crates/cgn-core/src/registry/store.rs`, change `RepoEntry`:
...
```bash
cargo test -p cgn-core registry::store::migration_tests 2>&1 | tail -10
```
...
```bash
cargo test -p cgn-core registry 2>&1 | tail -20
```
...
```bash
git add crates/cgn-core/src/registry/store.rs
...
**Files:**
- Create: `crates/cgn-cli/src/repo_selector.rs`
- Create: `crates/cgn-cli/tests/repo_selector.rs`
- Modify: `crates/cgn-cli/src/lib.rs` (add `pub mod repo_selector;`)
...
```rust
use cgn_cli::repo_selector::{parse, Atom, Selector};
...
```bash
cargo test -p cgn-cli --test repo_selector 2>&1 | tail -10
```
...
Update `crates/cgn-cli/src/lib.rs` (create if missing):
...
```bash
cat crates/cgn-cli/src/lib.rs 2>/dev/null
```
...
```bash
git add crates/cgn-cli/src/repo_selector.rs \
        crates/cgn-cli/src/lib.rs \
        crates/cgn-cli/tests/repo_selector.rs
...
**Files:**
- Modify: `crates/cgn-cli/src/repo_selector.rs`
- Modify: `crates/cgn-cli/tests/repo_selector.rs`

- [ ] **Step 1: Write failing resolver tests**

Append to `crates/cgn-cli/tests/repo_selector.rs`:

```rust
use cgn_cli::repo_selector::{resolve, ResolveError, ResolvedRepo};
use cgn_core::registry::{BranchEntry, GroupEntry, RegistryFile, RepoEntry};
...
```bash
cargo test -p cgn-cli --test repo_selector resolve 2>&1 | tail -10
```
...
Append to `crates/cgn-cli/src/repo_selector.rs`:

```rust
use cgn_core::registry::{RegistryFile, RepoEntry};
...
**Files:**
- Create: `crates/cgn-cli/src/hint.rs`
- Create: `crates/cgn-cli/tests/hint.rs`
- Modify: `crates/cgn-cli/src/lib.rs` (add `pub mod hint;`)
...
```rust
use cgn_cli::hint::{empty_result, error_with_cause, fuzzy_suggestions, stale_warning};

#[test]
fn empty_result_format() {
    let msg = empty_result("foo", "symbol", "cgn search foo --mode bm25");
    assert!(msg.contains("No"));
    assert!(msg.contains("foo"));
    assert!(msg.contains("cgn search"));
...
```bash
cargo test -p cgn-cli --test hint 2>&1 | tail -10
```
...
Add to `crates/cgn-cli/src/lib.rs`:
...
```bash
git add crates/cgn-cli/src/hint.rs \
        crates/cgn-cli/src/lib.rs \
        crates/cgn-cli/tests/hint.rs
...
**Files:**
- Create: `crates/cgn-cli/src/auto_ensure.rs`
- Create: `crates/cgn-cli/tests/auto_ensure.rs`
- Modify: `crates/cgn-cli/src/lib.rs`
...
```bash
cargo test -p cgn-cli --test auto_ensure 2>&1 | tail -10
```
...
```bash
grep -n "walkdir" crates/cgn-cli/Cargo.toml
```
...
git add crates/cgn-cli/src/auto_ensure.rs \
        crates/cgn-cli/src/lib.rs \
        crates/cgn-cli/tests/auto_ensure.rs \
...
- Create: `crates/cgn-cli/src/commands/admin/mod.rs`
...
Create `crates/cgn-cli/tests/cli_surface.rs`:
...
```bash
cargo test -p cgn-cli --test cli_surface 2>&1 | tail -15
```
...
pub fn run(cmd: AdminCommands) -> Result<(), cgn_core::CgnError> {
    match cmd {
...
Create `crates/cgn-cli/src/commands/admin/group.rs`:
...
pub fn run(_cmd: GroupCommands) -> Result<(), CgnError> {
    Err(CgnError::Output("admin group commands not implemented yet".into()))
}
```

- [ ] **Step 4: Add admin module to commands/mod.rs**

Modify `crates/cgn-cli/src/commands/mod.rs` — add line at top:
...
Replace the current `Commands` enum in `crates/cgn-cli/src/main.rs` with:
...
enum Commands {
    ...
    /// Internal: diff resolver dump against language oracle (cgn-dev QA)
    #[command(hide = true)]
...
Create `crates/cgn-cli/src/commands/scan.rs`:
...
pub fn run(_args: ScanArgs, _engine: &Engine) -> Result<(), CgnError> {
    Err(CgnError::Output("scan command stub — implement in Task 3.1".into()))
}
```

Create `crates/cgn-cli/src/commands/contracts.rs`:
...
pub fn run(_args: ContractsArgs, _engine: &Engine) -> Result<(), CgnError> {
    Err(CgnError::Output("contracts command stub — implement in Task 3.2".into()))
}
```

Add both modules to `crates/cgn-cli/src/commands/mod.rs`:
...
```bash
cargo build --bin cgn 2>&1 | tail -20
```
...
git add crates/cgn-cli/src/main.rs \
        ...
        crates/cgn-cli/src/commands/scan.rs \
        crates/cgn-cli/src/commands/contracts.rs \
        crates/cgn-cli/tests/cli_surface.rs
...
'cgn admin'. Old commands remain wired to existing Args types;
...
**Files:**
- Move: `crates/cgn-cli/src/commands/context.rs` → `crates/cgn-cli/src/commands/inspect.rs`
- Modify: `crates/cgn-cli/src/main.rs` (variant wire-up)
- Move: `crates/cgn-cli/tests/context_cmd.rs` → `crates/cgn-cli/tests/inspect_cmd.rs`
- Modify: `crates/cgn-cli/tests/inspect_cmd.rs` (new assertions for composition)

- [ ] **Step 1: Write failing tests for composition**

Open `crates/cgn-cli/tests/inspect_cmd.rs` (currently named context_cmd.rs) and add:
...
```bash
cargo test -p cgn-cli --test inspect_cmd 2>&1 | tail -15
```
...
```bash
git mv crates/cgn-cli/src/commands/context.rs crates/cgn-cli/src/commands/inspect.rs
git mv crates/cgn-cli/tests/context_cmd.rs crates/cgn-cli/tests/inspect_cmd.rs
```
...
Use `cgn rename` (or sed) to update all references — there should be ~5 places in inspect.rs itself.
...
```bash
cargo build --bin cgn 2>&1 | tail -3
cargo test -p cgn-cli --test inspect_cmd 2>&1 | tail -15
```
...
```bash
cargo test -p cgn-cli --test cli_surface 2>&1 | tail -10
```
...
```bash
git add -A crates/cgn-cli/src/commands/inspect.rs \
          crates/cgn-cli/src/commands/mod.rs \
          crates/cgn-cli/src/main.rs \
          crates/cgn-cli/tests/inspect_cmd.rs
git rm crates/cgn-cli/src/commands/context.rs 2>/dev/null || true
git rm crates/cgn-cli/tests/context_cmd.rs 2>/dev/null || true
...
**Files:**
- Move: `crates/cgn-cli/src/commands/query.rs` → `crates/cgn-cli/src/commands/search.rs`
- Modify: search.rs to add `--mode` and absorb `multi_query.rs` parallel pattern
- Delete: `crates/cgn-cli/src/commands/multi_query.rs`
- Delete: `crates/cgn-cli/tests/multi_query_cmd.rs`
- Move: tests query_cmd.rs → search_cmd.rs (merge multi_query tests)

- [ ] **Step 1: Write failing tests**

Create or modify `crates/cgn-cli/tests/search_cmd.rs`:
...
```bash
cargo test -p cgn-cli --test search_cmd 2>&1 | tail -15
```
...
```bash
git mv crates/cgn-cli/src/commands/query.rs crates/cgn-cli/src/commands/search.rs
git mv crates/cgn-cli/tests/query_cmd.rs crates/cgn-cli/tests/search_cmd.rs 2>/dev/null || true
```
...
fn detect_mode(input: &str, embeddings_available: bool) -> SearchMode {
    ...
    if embeddings_available {
        ...
        eprintln!("→ falling back to bm25 (no embeddings — build with `cgn admin index --embeddings`)");
        SearchMode::Bm25
    }
}
```
...
Open `crates/cgn-cli/src/commands/multi_query.rs` and copy the rayon parallel-load + top-K heap merge logic into `search.rs::run`. Pseudocode:

```rust
pub fn run(args: SearchArgs, _engine: &Engine) -> Result<(), CgnError> {
    let registry = registry::load()?;
    let cwd = std::env::current_dir().map_err(|e| CgnError::Io(e.to_string()))?;
...
```bash
git rm crates/cgn-cli/src/commands/multi_query.rs
git rm crates/cgn-cli/tests/multi_query_cmd.rs
```
...
- Modify: `crates/cgn-cli/tests/` — relevant impact tests

- [ ] **Step 1: Write failing tests**

In `crates/cgn-cli/tests/impact_cmd.rs` (create if missing):
...
```bash
cargo test -p cgn-cli --test impact_cmd 2>&1 | tail -10
```
...
```bash
cargo build --bin cgn 2>&1 | tail -3
cargo test -p cgn-cli --test impact_cmd 2>&1 | tail -15
```
...
```bash
git add crates/cgn-cli/src/commands/impact.rs \
        crates/cgn-cli/tests/impact_cmd.rs
...
**Files:**
- Delete: `crates/cgn-cli/src/commands/detect_changes.rs`
- Delete: `crates/cgn-cli/tests/detect_changes.rs`
...
```bash
./target/release/cgn impact --since HEAD~1 --repo . 2>&1 | head -20
```

Expected: similar output shape to old `cgn detect_changes`.

- [ ] **Step 2: Delete module + test**

```bash
git rm crates/cgn-cli/src/commands/detect_changes.rs
git rm crates/cgn-cli/tests/detect_changes.rs
```
...
```bash
cargo build --bin cgn 2>&1 | grep -E "error|warning: unused" | head -10
```
...
```bash
cargo test -p cgn-cli --test coverage_cmd 2>&1 | tail -10
```
...
```rust
//! `cgn coverage` — unified health report. Folds the old commands:
...
pub fn run(args: CoverageArgs, graph_path_global: &std::path::Path) -> Result<(), CgnError> {
    let mut sections: Vec<(&str, Value)> = vec![];

    // Section 1: registry overview (always, if no --repo or aggregated)
    let registry = cgn_core::registry::load()
        .map_err(|e| CgnError::Output(format!("registry load: {e}")))?;
    ...
    if let Some(repo_sel) = args.repo.as_deref() {
        ...
        let selector = crate::repo_selector::parse(repo_sel)
            .map_err(|e| CgnError::Output(format!("selector parse: {e}")))?;
        let resolved = crate::repo_selector::resolve(&selector, &registry, cwd.to_str().unwrap_or("."))
            .map_err(|e| CgnError::Output(format!("selector resolve: {e}")))?;
...
fn build_registry_overview(registry: &cgn_core::registry::RegistryFile, detailed: bool) -> Value {
...
fn build_groups_overview(registry: &cgn_core::registry::RegistryFile) -> Value {
...
fn fetch_frameworks(_r: &crate::repo_selector::ResolvedRepo) -> Value {
    ...
    // shared helper in cgn-core if it isn't already.
    json!({ "detected": [] })
}
...
2. Extract their core analysis into reusable functions (likely in `cgn-core`).
...
```bash
git rm crates/cgn-cli/src/commands/doctor.rs \
       crates/cgn-cli/src/commands/status.rs \
       crates/cgn-cli/src/commands/list.rs \
       crates/cgn-cli/src/commands/summarize.rs \
       crates/cgn-cli/src/commands/tool_map.rs
git rm crates/cgn-cli/tests/doctor_cmd.rs 2>/dev/null || true
# Remove other test files referencing the deleted modules:
grep -l "use.*doctor\|use.*status\|use.*list\|use.*summarize\|use.*tool_map" crates/cgn-cli/tests/*.rs
```
...
```bash
cargo build --bin cgn 2>&1 | tail -3
cargo test -p cgn-cli --test coverage_cmd 2>&1 | tail -15
cargo test -p cgn-cli --test cli_surface 2>&1 | tail -10
```
...
**Files:**
- Create: `crates/cgn-cli/src/commands/routes.rs`
- Delete: route_map.rs, api_impact.rs
- Modify: main.rs + mod.rs

- [ ] **Step 1: Write failing tests**

Create `crates/cgn-cli/tests/routes_cmd.rs`:
...
```bash
cargo test -p cgn-cli --test routes_cmd 2>&1 | tail -10
```
...
fn list_routes(engine: &Engine, method: Option<&str>) -> Result<(), CgnError> {
    // Port body of old route_map::run, with `method` filter
    // ... (see commands/route_map.rs)
    Ok(())
}

fn inspect_route(engine: &Engine, path: &str, method: Option<&str>) -> Result<(), CgnError> {
...
```bash
git rm crates/cgn-cli/src/commands/route_map.rs \
       crates/cgn-cli/src/commands/api_impact.rs
git rm crates/cgn-cli/tests/api_impact_cmd.rs 2>/dev/null || true
```
...
```bash
cargo build --bin cgn 2>&1 | tail -3
cargo test -p cgn-cli --test routes_cmd 2>&1 | tail -15
```
...
**Files:**
- Modify: `crates/cgn-cli/src/commands/rename.rs`
- Modify: `crates/cgn-cli/tests/rename_cmd.rs`

- [ ] **Step 1: Write failing tests**

In `crates/cgn-cli/tests/rename_cmd.rs`:
...
```bash
cargo test -p cgn-cli --test rename_cmd 2>&1 | tail -15
```
...
```rust
pub fn run(args: RenameArgs, engine: &Engine) -> Result<(), CgnError> {
    ...
    if !residuals.is_empty() {
        let opts: Vec<&str> = residuals.iter()
            ...
            .collect();
        if !opts.is_empty() {
            eprintln!("→ To also rename in those: cgn rename {} {} {}",
...
```bash
git add crates/cgn-cli/src/commands/rename.rs \
        crates/cgn-cli/tests/rename_cmd.rs
...
In `crates/cgn-cli/tests/cypher_content.rs` (or create `cypher_cmd.rs`):
...
```bash
cargo build --bin cgn 2>&1 | tail -3
cargo test -p cgn-cli --test cypher_content 2>&1 | tail -5
```
...
git add crates/cgn-cli/src/commands/cypher.rs \
        crates/cgn-cli/tests/cypher_content.rs
...
**Files:**
- Modify: `crates/cgn-cli/src/commands/scan.rs` (replaces stub from Task 1.1)
- Create: `crates/cgn-cli/tests/scan_cmd.rs`
...
```bash
cargo test -p cgn-cli --test scan_cmd 2>&1 | tail -10
```
...
pub fn run(args: ScanArgs, engine: &Engine) -> Result<(), CgnError> {
    // 1. Parse file via tree-sitter (pick language by extension)
    let source = std::fs::read_to_string(&args.file)
        .map_err(|e| CgnError::Io(format!("read {}: {e}", args.file)))?;
    let lang = detect_language(&args.file)
        .ok_or_else(|| CgnError::Output(hint::error_with_cause(
            &format!("scan failed: unknown language for {}", args.file),
            "extension not in supported list",
            "supported extensions are listed in `cgn coverage --blind-spots`",
        )))?;

    let refs = extract_identifier_references(&source, lang)
        .map_err(|e| CgnError::Output(hint::error_with_cause(
            &format!("scan failed: cannot parse {}", args.file),
            &e,
            "verify file syntax or run `cgn coverage --blind-spots`",
        )))?;
...
fn extract_identifier_references(_source: &str, _lang: &str) -> Result<Vec<Reference>, String> {
    ...
    // Use cgn_analyzer's identifier_finder per language (see
    // crates/cgn-analyzer/src/identifier_finder/<lang>.rs) to extract
    // all identifier references.
    Ok(vec![])
}

fn symbol_exists(_graph: &cgn_core::graph::ArchivedZeroCopyGraph, _name: &str) -> bool {
    // Iterate graph nodes, return true if any matches name
    false
}

fn fuzzy_match_symbols(
    _graph: &cgn_core::graph::ArchivedZeroCopyGraph,
    _query: &str,
    _top_k: usize,
) -> Vec<String> {
...
- `crates/cgn-analyzer/src/identifier_finder/` for per-language ref extraction
- `cgn_core::graph` for node iteration
...
```bash
cargo build --bin cgn 2>&1 | tail -3
cargo test -p cgn-cli --test scan_cmd 2>&1 | tail -15
```

- [ ] **Step 5: Commit**

```bash
git add crates/cgn-cli/src/commands/scan.rs \
        crates/cgn-cli/tests/scan_cmd.rs
...
**Files:**
- Modify: `crates/cgn-cli/src/commands/contracts.rs` (replaces stub)
- Create: `crates/cgn-cli/tests/contracts_cmd.rs`
...
pub fn run(args: ContractsArgs, _engine: &Engine) -> Result<(), CgnError> {
...
fn scan_contracts_in_repo(
    ...
    _kind_filter: &str,
) -> Result<(Vec<ContractRef>, Vec<ContractRef>), CgnError> {
    // TODO during implementation:
...
Fill TODOs by referencing `cgn-core`'s Route node type and FETCHES edge type. The matching algorithm is straightforward: equality on `path_or_topic` (with optional kind tagging).

- [ ] **Step 4: Build + tests**

```bash
cargo build --bin cgn 2>&1 | tail -3
cargo test -p cgn-cli --test contracts_cmd 2>&1 | tail -15
```

- [ ] **Step 5: Commit**

```bash
git add crates/cgn-cli/src/commands/contracts.rs \
        crates/cgn-cli/tests/contracts_cmd.rs
...
```bash
git mv crates/cgn-cli/src/commands/clean.rs        crates/cgn-cli/src/commands/admin/drop.rs
git mv crates/cgn-cli/src/commands/init.rs         crates/cgn-cli/src/commands/admin/install_hook.rs
git mv crates/cgn-cli/src/commands/analyze.rs      crates/cgn-cli/src/commands/admin/index.rs
git mv crates/cgn-cli/src/commands/prune.rs        crates/cgn-cli/src/commands/admin/prune.rs
git mv crates/cgn-cli/src/commands/rename_branch.rs crates/cgn-cli/src/commands/admin/rename_branch.rs
git mv crates/cgn-cli/src/commands/config.rs       crates/cgn-cli/src/commands/admin/config.rs
git rm crates/cgn-cli/src/commands/index.rs
git rm crates/cgn-cli/src/commands/remove.rs
git rm crates/cgn-cli/src/commands/analyze_here.rs
```
...
Run `cgn rename` (using current build) per name (in dry-run first):

```bash
./target/release/cgn rename CleanArgs DropArgs --dry-run
./target/release/cgn rename CleanArgs DropArgs
# similarly for InitArgs→InstallHookArgs, AnalyzeArgs→IndexArgs
```

Or use `sd` / `sed` per file if `cgn rename` isn't ready.
...
**Files:**
- Modify: `crates/cgn-cli/src/commands/admin/group.rs`
- Create: `crates/cgn-cli/tests/admin_group_cmd.rs`
...
fn add(repo: &str, group: &str) -> Result<(), CgnError> {
    let path = cgn_core::registry::default_path();
    let mut reg = cgn_core::registry::RegistryFile::read_or_empty(&path)
        .map_err(|e| CgnError::Io(format!("registry read: {e}")))?;

    // Update repo's groups list
    let r = reg.repos.iter_mut()
        .find(|r| r.name == repo)
        .ok_or_else(|| CgnError::Output(format!("repo not found: {repo}")))?;
    ...
    if let Some(g) = reg.groups.iter_mut().find(|g| g.name == group) {
        ...
        reg.groups.push(cgn_core::registry::GroupEntry {
...
```bash
cargo build --bin cgn 2>&1 | tail -3
cargo test -p cgn-cli --test admin_group_cmd 2>&1 | tail -15
```

- [ ] **Step 5: Commit**

```bash
git add crates/cgn-cli/src/commands/admin/group.rs \
        crates/cgn-cli/tests/admin_group_cmd.rs
...
```bash
git rm crates/cgn-cli/src/commands/cluster.rs \
       crates/cgn-cli/src/commands/process.rs
git rm crates/cgn-cli/tests/cluster_cmd.rs 2>/dev/null || true
git rm crates/cgn-cli/tests/process_cmd.rs 2>/dev/null || true
```
...
```bash
cargo build --bin cgn 2>&1 | grep -E "warning: unused|error" | head -20
```
...
```bash
grep -nE "cgn (analyze|clean|init|list|index|config|remove|register|context|query|doctor|status|summarize|tool_map|tool-map|route_map|route-map|api_impact|api-impact|detect_changes|detect-changes|cluster|process|multi_query|multi-query|analyze-here|analyze_here)" README.md
```
...
| Old | New |
|---|---|
| `cgn analyze` | `cgn admin index` |
| `cgn analyze --repo .` | `cgn admin index .` |
| `cgn analyze-here` | (drop — auto-ensure) |
| `cgn clean` | `cgn admin drop` |
| `cgn remove` | `cgn admin drop` (combined) |
| `cgn init` | `cgn admin install-hook` |
| `cgn list` | `cgn coverage` |
| `cgn index` (recovery) | (drop — re-run `cgn admin index`) |
| `cgn config` | `cgn admin config` |
| `cgn prune` | `cgn admin prune` |
| `cgn rename-branch` | `cgn admin rename-branch` |
| `cgn verify-resolver` | (hidden — internal) |
| `cgn context` | `cgn inspect` |
| `cgn query` | `cgn search` |
| `cgn doctor` | `cgn coverage` |
| `cgn status` | `cgn coverage` |
| `cgn summarize` | `cgn coverage` |
| `cgn tool_map` / `cgn tool-map` | `cgn coverage --detailed` (externals section) |
| `cgn route_map` / `cgn route-map` | `cgn routes` |
| `cgn api_impact /path` / `cgn api-impact /path` | `cgn routes /path` |
| `cgn detect_changes` / `cgn detect-changes` | `cgn impact --since HEAD~1` |
| `cgn multi_query` / `cgn multi-query` | `cgn find --batch` (single repo, BM25; reads patterns from stdin) or `cgn group find --batch` (multi-repo). Batch is a flag on the existing verb, not a separate CLI. |
| `cgn cluster` / `cgn process` | `cgn cypher` |
...
commands** (down from 25), **7 admin commands** under `cgn admin`, plus
a global `--repo` selector supporting `@group` / `@all` / ad-hoc CSV.
Cross-repo search merged into `cgn search`; `cgn multi_query` removed.
See `docs/specs/2026-05-15-cgn-cli-redesign-design.md` for the full
spec and rationale.
```
...
**Files:**
- Search for `cgn` references in `.claude/`, `docs/`, and any in-repo `CLAUDE.md`

- [ ] **Step 1: Locate refs**

```bash
find . -name "CLAUDE.md" -o -name "*.md" 2>/dev/null | xargs grep -l "cgn context\|cgn query\|cgn analyze\|cgn doctor\|cgn status\|cgn clean" 2>/dev/null
```
...
- Modify: `scripts/benchmark_cgn.py`

- [ ] **Step 1: Find refs**

```bash
grep -rn "cgn analyze\|cgn context\|cgn query\|cgn doctor\|cgn clean\|cgn route_map\|cgn api_impact\|cgn multi_query\|cgn detect_changes" scripts/
```
...
# E.g. for benchmark_cgn.py:
python scripts/benchmark_cgn.py --help
```
...
```bash
./target/release/cgn --help | head -20
./target/release/cgn admin --help | head -10
./target/release/cgn inspect --help | head -10
```
...
See docs/specs/2026-05-15-cgn-cli-redesign-design.md."
```
...
- [ ] All 9 agent commands present in `cgn --help`: inspect, search, impact, rename, cypher, coverage, routes, scan, contracts
- [ ] `cgn --help` does NOT mention `admin`
- [ ] `cgn admin --help` lists: install-hook, drop, prune, rename-branch, config, group, index (7 entries)
- [ ] `cgn admin group --help` lists: add, remove
- [ ] Hidden commands runnable: `cgn hook-handle`, `cgn hook-watcher`, `cgn verify-resolver` (no `--help` mention)
