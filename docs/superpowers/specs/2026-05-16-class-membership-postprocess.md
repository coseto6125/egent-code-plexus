...
2. 把 Class node 依 `start_line` 升序、`end_line` 降序排序（外層 Class 排前面）
3. 對每個 Method / Property，找「最小包含其 span 的 Class」（innermost enclosing），emit `HasMethod` / `HasProperty` edge
...
採嚴格包含（strict `<` 在外，等於允許在內）— 同 line 起始但不同 column 也算包含；同 span 不算自包含。
...
理由：必須在所有 file 的 node 都 inserted 後跑（cross-file 不適用 — Class 跟其 method 必同檔，但 post-process 仍需完整 graph view），且必須在 serialize 前 mutate。
...
| 語言 | Fixture | 斷言 |
|---|---|---|
| Rust | `struct Foo { a: i32 } impl Foo { fn bar(&self) {} }` | `inspect Foo` 回 `contained_methods: [bar]`, `contained_properties: [a]` |
...
| Spec 跑出未預期的 edge 爆炸（每 class 數十 methods × N classes） | 低 | Complexity O(N²) per file 但常數小；先測 worst-case fixture（PyTorch-like 大 class）驗證 |
