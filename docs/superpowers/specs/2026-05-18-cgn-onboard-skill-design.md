...
Ship a **personalized installation + configuration wizard** for `code-graph-nexus`
...
from "no cgn installed" → "cgn ready + indexed + grouped + MCP wired +
...
- **CLI reference dynamically generated per cgn version**, not hand-written.
- **Batch-apply flow**: collect choices first, run background install, then
  apply all `cgn` actions once binary is verified.
...
│   └── cgn-onboard/                               ← NEW — onboarding SKILL pack
│       │
...
│   ├── gen-cli-ref.sh                             ← scrapes `cgn <cmd> --help` → _shared/cli/<ver>/
...
│
└── .github/workflows/
    ├── skill-aggregate.yml                        ← NEW — regenerates ONBOARDING.md on push (touching docs/skills/cgn-onboard/)
    └── skill-cli-ref.yml                          ← NEW — regenerates _shared/cli/<ver>/ on code-graph-nexus release tag
```
...
- **Zero version drift**: cgn CLI and the SKILL ship in lockstep; § 4 D
...
- **`gen-cli-ref.sh` is trivial**: builds `cgn` locally with `cargo build`
...
- Trade-off: (c)/(d) outlets pull ~hundreds of MB of Rust source. Mitigated
  ...
  git sparse-checkout set docs/skills/cgn-onboard` — documented in README.
...
1. **Core directives** (3–4 items): every step recommends + user picks
   ...
   `cgn` CLI; **never block on background install — collect later phases'
   choices in parallel**.
...
|---|---|---|
| `01-install.md` | Detect OS/arch/pkg manager; recommend best install path; **trigger background download**; verify `cgn --version` at T6 | Installed binary |
| `02-first-index.md` | Ask which repo(s) to index; record choice (no `cgn` call yet) | Choice queued |
...
| `05-summary.md` | Drain queued choices in batch at T7; persist `~/.cgn/onboarding-summary.md`; emit recommendation list | Final state |
...
- **`cli/<version>/<cmd>.md`** — ~20–40 lines per command card. Auto-generated
  by `tools/gen-cli-ref.sh` from `cgn <cmd> --help`. Pull when needing exact
  flag syntax.
...
T5  Phase 04 mcp: agent detects installed IDEs; confirms which configs
        ...
        ▼
T6  ─── Batch-apply gate ─────────────────────────────────
        Wait for background download to complete + verify cgn --version.
        On failure → § 4 path B (hypotheses + retry / change / skip).
        On success → proceed to T7.
        │
        ▼
T7  Batch-apply queued choices in order:
        cgn admin index --repo <choices from T3>
        cgn admin group add --repo <...> <group-name>  (per T4)
        write IDE MCP configs (per T5)
        verify each command succeeded before moving to next
        │
        ▼
T8  Phase 05 summary:
        write ~/.cgn/onboarding-summary.md (YAML frontmatter + checklist)
...
| Chat mentions `cargo` / `rust` / `cargo workspace` | `install_pref = cargo-binstall` | Recommend `cargo binstall code-graph-nexus` |
...
Based on your persona (Rust power user, macOS arm64), recommendation:
  ...
  ✓ Recommended: cargo binstall code-graph-nexus
     Why: cargo-binstall detected on your system; no compile, ~30s

  Alternative A: brew install <tap>/code-graph-nexus
...
| `~/.cgn/onboarding-summary.md` | Recipient's local disk | Future sessions can grep for "last install state" |
| `~/bin/cgn`, `~/.cgn/code-graph-nexus*/`, `registry.json` | Recipient's local disk | Owned by cgn CLI itself; wizard only triggers it |
...
| **B. Install / command fails** | `cargo binstall ... → exit 1`, `cgn admin index ... → exit 1`, network timeout | (1) Paste stderr verbatim. (2) Map against "common-cause table" → offer 1–3 hypotheses. (3) Present **retry / change-method / skip**. Never auto-retry, never silently switch methods. |
...
| **D. cgn CLI behavior diverges** | `cgn` output format changed, flag missing, version mismatch | Do not hardcode flags. First call `cgn <cmd> --help` for ground truth. If divergent → set `skill_drift_detected = true`, print diff, suggest user file a report |
...
| first-index | `cgn admin index ... → not a git repo` | Wrong path / no `.git` |
| first-index | index takes >3min | Large repo / vendored / should configure `.cgnignore` |
| group | `cgn admin group add ... → repo not in registry` | Repo not yet indexed |
...
1. **New-session preamble** — directive 1 in SKILL.md: "If
   `~/.cgn/onboarding-summary.md` exists, read it first."
...
- ❌ Don't touch any user file other than `~/.cgn/onboarding-summary.md`
  (do not modify `~/.zshrc`, `~/.gitconfig`, `~/.cursor/...` unless phase 4
  mcp explicitly with user consent).
- ❌ Don't assume "future cgn versions should have flag X" — check live `--help`.
...
| All `cgn <subcmd>` references in Layer 1+2 exist in latest CLI manifest | Stale reference |
...
### T3 — CLI ref drift (after cgn version bump)

```bash
cgn --version
./tools/gen-cli-ref.sh > /tmp/cli-ref-new/
diff -r skills/cgn-onboard/_shared/cli/<ver>/ /tmp/cli-ref-new/
```
...
5. ... walk all 5 phases
6. verify: ~/.cgn/onboarding-summary.md exists and contains all 5 phases
7. verify: cgn find . in indexed repo returns results
...
| (c) Plugin | Clone to `~/.claude/plugins/cgn-onboard/` → smoke playbook passes |
| (d) git clone | Clone → `tree skills/cgn-onboard/` structure equals source |
...
- **Wizard for `cgn admin` advanced flags** (`--force`, `--baseline`, etc.):
  current 5-phase flow covers happy paths only.
- **Auto-update of installed cgn**: phase 01 install, not "upgrade an
  existing cgn to the latest". Future phase 06 could handle.
