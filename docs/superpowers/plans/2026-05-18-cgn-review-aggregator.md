# `cgn review` Aggregator Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Replace 6 individual CLI commands (`scan`, `impact`, `coverage`, `tool-map`, `shape-check`, `diff`) with a single `cgn review` aggregator. `scan` is hard-deleted (signal:noise too low per spec). The other 5 become Rust-library-only and are orchestrated from `review`.

**Architecture:**
- Each constituent (`impact` / `coverage` / `shape_check` / `tool_map`→`egress` / `diff`) is split into two layers: a `build_payload(args, engine) -> Result<Value, CgnError>` library fn (pure data, no clap, no emit) and (Phase 1) a thin CLI wrapper that calls it then emits. Phase 3 removes the CLI wrappers; the library fns stay.
...
**Spec:** `docs/feat/2026-05-17-cgn-review-spec.md`
...
- `crates/cgn-cli/src/commands/scan_filters.rs`
...
```bash
rg -n '"scan"|scan_filters' crates/cgn-cli/tests/
```

### Task 0.2: Failing test that confirms scan is gone

**Files:**
- Modify: `crates/cgn-cli/tests/cli_surface.rs`

- [ ] **Step 1: Add a regression test that `cgn --help` does NOT list scan**
...
```bash
cargo test -p code-graph-nexus --test cli_surface top_level_does_not_list_scan
```
...
**Files:**
- Delete: `crates/cgn-cli/src/commands/scan.rs`
- Delete: `crates/cgn-cli/src/commands/scan_filters.rs`
- Delete: `crates/cgn-cli/tests/scan_cmd.rs`
- Delete: any other `crates/cgn-cli/tests/scan_*` files found in Task 0.1

- [ ] **Step 1: Delete files**

```bash
git rm crates/cgn-cli/src/commands/scan.rs \
       crates/cgn-cli/src/commands/scan_filters.rs \
       crates/cgn-cli/tests/scan_cmd.rs
...
**Files:**
- Modify: `crates/cgn-cli/src/main.rs:69` (variant), `:165` (repo dispatch), `:204` (run dispatch)
...
```bash
rg -n '^pub mod scan' crates/cgn-cli/src/commands/mod.rs
```
...
```bash
rg -n 'use crate::commands::scan' crates/cgn-cli/src/
```

### Task 0.6: `identifier_finder` — keep (verified consumer: `cgn rename`)

**Context:** Original plan assumed scan was the only consumer. Verification (`rg -n 'identifier_finder' crates/`) shows `crates/cgn-cli/src/commands/rename.rs:22` calls `find_identifier_occurrences`. The module stays.
...
**Files:** discovered in Task 0.1 (excluding `docs/feat/2026-05-17-cgn-review-spec.md` — that's the spec, leave it).

- [ ] **Step 1: For each doc file, remove `cgn scan` mentions, replace with brief deprecation note in the relevant section**

Search:
```bash
rg -n 'cgn scan|cgn_scan' docs/ README*.md .claude/skills/ 2>/dev/null
```
...
```bash
git add -A
git commit -m "refactor(cli): hard-delete cgn scan + scan_filters + identifier_finder

Signal:noise was ~1:10 even with stdlib filters; language-native
linters cover the actionable subset. Removed:
- cgn scan subcommand (CLI + MCP via clap auto-derive)
- scan_filters module
...
- Add `pub fn build_payload(args: &XxxArgs, engine: &Engine) -> Result<serde_json::Value, CgnError>` (pure — no `emit`, no `println!`)
- Keep `pub fn run(args: XxxArgs, engine: &Engine) -> Result<(), CgnError>` as a 3-line wrapper:
  ```rust
...
- Test: `crates/cgn-cli/tests/impact_build_payload.rs` (new)

- [ ] **Step 1: Write a golden-payload test using a fixture repo**

```rust
// crates/cgn-cli/tests/impact_build_payload.rs
use cgn_cli::commands::impact::{build_payload, ImpactArgs, Direction};
// (path adjusts to actual crate name `cgn_cli`)
...
(If a `test_helpers` module doesn't yet exist for fixture-loading, mirror whatever pattern existing impact tests use — `crates/cgn-cli/tests/impact_*.rs` files.)

- [ ] **Step 2: Run — fails (build_payload undefined)**

```bash
cargo test -p code-graph-nexus --test impact_build_payload
```
...
In `impact.rs`:
- Move the body of `run` (after the `args.target` fold-in lines) into a new `pub fn build_payload(args: &mut ImpactArgs, engine: &Engine) -> Result<Value, CgnError>` that returns the final `Value` instead of calling `emit`.
- Replace `emit(&result, format)` and `emit(&value, format)` call sites with `return Ok(result);` / `return Ok(value);`.
- Make `run` become:
  ```rust
  pub fn run(mut args: ImpactArgs, engine: &Engine) -> Result<(), CgnError> {
  ...
  ```
- The `impact_by_name` / `impact_with_baseline` helper signatures change to return `Result<Value, CgnError>` instead of calling `emit` themselves.

- [ ] **Step 4: Run new test + existing impact tests**

```bash
cargo test -p code-graph-nexus --test impact_build_payload
cargo test -p code-graph-nexus --test 'impact*'
```
...
Preparation for cgn review aggregator (Phase 2). No behavior change.
...
- Test: `crates/cgn-cli/tests/coverage_build_payload.rs` (new)

- [ ] **Step 1: Failing golden-payload test**

```rust
// crates/cgn-cli/tests/coverage_build_payload.rs
use cgn_cli::commands::coverage::{build_payload, CoverageArgs};
...
- [ ] **Step 3: Refactor — extract `pub fn build_payload(args: &CoverageArgs, _graph_arg: &Path) -> Result<Value, CgnError>` containing everything currently between `let format = OutputFormat::parse(...)` and `emit(&value, format)`. Have `run` call `build_payload` then `emit`.**

- [ ] **Step 4: All coverage tests pass**

```bash
cargo test -p code-graph-nexus --test 'coverage*'
cargo test -p code-graph-nexus --test blind_spots_python
```
...
**Files:**
- Modify: `crates/cgn-cli/src/commands/shape_check.rs:48`

`shape_check.rs` already has `fn build_payload(args: ShapeCheckArgs, engine: &Engine) -> Result<Value, CgnError>`. Just promote to `pub` and change signature to take `&ShapeCheckArgs`.
...
**Files:**
- Modify: `crates/cgn-cli/src/commands/tool_map.rs:150`

- [ ] **Step 1: Failing test**
- [ ] **Step 2: Run — fails**
- [ ] **Step 3: Mirror Task 1.1 mechanics — extract body of `run` into `pub fn build_payload(args: &ToolMapArgs, engine: &Engine) -> Result<Value, CgnError>`. Skip the `emit` at end; have `run` call `build_payload` then `emit`.**
- [ ] **Step 4: All tool_map tests pass**

```bash
cargo test -p code-graph-nexus --test 'tool_map*'
```
...
**Files:**
- Modify: `crates/cgn-cli/src/commands/diff/mod.rs:51`
...
- New library fn: `pub fn build_payload(args: &DiffArgs) -> Result<DiffPayload, CgnError>` where `DiffPayload` is a struct holding the three optional diffs + baseline/current SHAs.
...
```bash
cargo test -p code-graph-nexus --test 'diff*'
```
...
**Files:**
- Modify: `crates/cgn-cli/src/lib.rs` (or `main.rs` if no `lib.rs`)
- Modify: `crates/cgn-cli/Cargo.toml` if needed
...
```bash
rg -n '^pub mod commands' crates/cgn-cli/src/main.rs crates/cgn-cli/src/lib.rs 2>/dev/null
```
...
## Phase 2 — Build `cgn review` aggregator

### Task 2.1: Create review module skeleton

**Files:**
- Create: `crates/cgn-cli/src/commands/review/mod.rs`
- Create: `crates/cgn-cli/src/commands/review/findings.rs`
- Create: `crates/cgn-cli/src/commands/review/scope.rs`
- Create: `crates/cgn-cli/src/commands/review/aggregate.rs`
- Modify: `crates/cgn-cli/src/commands/mod.rs` — add `pub mod review;`

- [ ] **Step 1: Create empty module files**

```bash
mkdir -p crates/cgn-cli/src/commands/review
```

Write `crates/cgn-cli/src/commands/review/mod.rs`:
...
Add `pub mod review;` to `crates/cgn-cli/src/commands/mod.rs`.

### Task 2.2: Findings type

**Files:**
- Modify: `crates/cgn-cli/src/commands/review/findings.rs`

- [ ] **Step 1: Write failing unit test inside findings.rs**

```rust
// crates/cgn-cli/src/commands/review/findings.rs
use serde::Serialize;
...
```bash
cargo test -p code-graph-nexus --lib commands::review::findings
```
...
```bash
git add crates/cgn-cli/src/commands/review/
...
**Files:**
- Modify: `crates/cgn-cli/src/commands/review/scope.rs`
...
- [ ] **Step 3: Implement `pub fn resolve(args: &ReviewArgs, repo_dir: &Path) -> Result<Vec<PathBuf>, CgnError>`**
...
**Files:**
- Modify: `crates/cgn-cli/src/commands/review/aggregate.rs`
...
- [ ] **Step 3: Implement `pub fn run(files: &[PathBuf], repo_dir: &Path, engine: &Engine) -> Result<Report, CgnError>`**
...
pub fn run(files: &[PathBuf], repo_dir: &Path, engine: &Engine) -> Result<Report, CgnError> {
...
fn build_impact(files: &[PathBuf], engine: &Engine) -> Result<serde_json::Value, CgnError> { /* ... */ todo!() }
fn map_impact(v: &serde_json::Value) -> Result<Vec<Finding>, CgnError> { /* filter risk_level >= medium */ todo!() }
// ... etc
```
...
```bash
cargo test -p code-graph-nexus --lib commands::review::aggregate
```

- [ ] **Step 6: Commit incrementally — one commit per constituent helper.**

### Task 2.5: Wire `cgn review` into main.rs
...
```bash
cargo build -p code-graph-nexus --bin cgn --release
./target/release/cgn review --help
./target/release/cgn review --files crates/cgn-cli/src/main.rs --format json | head -20
```
...
**Files:**
- Modify: `crates/cgn-cli/tests/cli_surface.rs`
- Create: `crates/cgn-cli/tests/review_e2e.rs`

- [ ] **Step 1: Update cli_surface to assert review appears**

Add `"review"` to the agent-commands list.

- [ ] **Step 2: End-to-end test — fixture repo, real `cgn review` invocation, assert spec-compliant output**

```rust
// crates/cgn-cli/tests/review_e2e.rs
#[test]
...
```bash
cargo test -p code-graph-nexus --test review_e2e
cargo test -p code-graph-nexus --test cli_surface
```
...
**Files:**
- Rename: `crates/cgn-cli/src/commands/tool_map.rs` → `egress.rs`
- Modify: `crates/cgn-cli/src/commands/mod.rs` — `pub mod tool_map;` → `pub mod egress;`
- Modify: `crates/cgn-cli/src/commands/review/aggregate.rs` — replace `commands::tool_map` with `commands::egress`
- Modify: any test files referencing `tool_map`

- [ ] **Step 1: `git mv` the file**

```bash
git mv crates/cgn-cli/src/commands/tool_map.rs \
       crates/cgn-cli/src/commands/egress.rs
```
...
```bash
rg -n 'cgn (impact|coverage|tool-map|shape-check|diff)\b' README*.md docs/ .claude/skills/ 2>/dev/null
```

- [ ] **Step 2: For each hit: either update to mention `cgn review` instead, or remove the line entirely if the original example doesn't have a `cgn review` equivalent.**
