# cgn-mcp Crate Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Ship `cgn mcp serve` as a working MCP server (stdio JSON-RPC) that exposes the 8 core cgn commands as auto-discovered MCP tools, with dual dispatch modes (spawn default, daemon opt-in).

**Architecture:** New `cgn-mcp` crate provides the server library. The existing `cgn-cli` adds a `cgn mcp` subcommand wrapping it. Tool discovery is fully dynamic via the `inventory` crate — each `commands/*.rs` opts in by adding one `cgn_register_mcp_tool!` macro line. Daemon mode loads `Engine` once and refreshes via mtime-remap; spawn mode `Command::spawn`s a fresh `cgn <subcmd>` per call.
...
- `cgn admin` TUI shell (subproject B)
...
```
crates/cgn-mcp/                     ← NEW CRATE
├── Cargo.toml
├── src/
│   ├── lib.rs                              ← module re-exports
│   ├── registry.rs                         ← CgnMcpTool struct + inventory collect + name derivation
...
│   │   ├── mcp.rs                          ← NEW: cgn mcp serve/tools subcommand
...
└── Cargo.toml                              ← MODIFY: add cgn-mcp + schemars deps
```
...
**Files:**
- Modify: `crates/cgn-cli/src/output.rs`

- [ ] **Step 1: Write the failing test**

Add to bottom of `crates/cgn-cli/src/output.rs`:
...
```bash
cargo test -p code-graph-nexus --lib output::tests
```
...
Replace the existing `emit` function in `crates/cgn-cli/src/output.rs` with the pair:
    ...
    match format {
        OutputFormat::Toon => {
            ...
            _etoon::toon::encode(&bytes)
                .map_err(|e| CgnError::Output(format!("toon encode: {e}")))
        }
...
```bash
git add crates/cgn-cli/src/output.rs
...
### Task 2: Create `cgn-mcp` crate scaffold

**Files:**
- Create: `crates/cgn-mcp/Cargo.toml`
- Create: `crates/cgn-mcp/src/lib.rs`
- Modify: `Cargo.toml` (workspace root — add to `members`)
- Modify: `crates/cgn-cli/Cargo.toml` (add `schemars` dep — needed by upcoming command refactors)

- [ ] **Step 1: Write `Cargo.toml`**

```toml
# crates/cgn-mcp/Cargo.toml
[package]
name = "cgn-mcp"
version = "0.1.0"
edition = "2024"
...
```rust
// crates/cgn-mcp/src/lib.rs
//! MCP server library backing `cgn mcp serve`. Built around `inventory`
//! for zero-hardcode tool discovery; each cgn CLI command opts in via
//! `cgn_register_mcp_tool!`.
...
```bash
mkdir -p crates/cgn-mcp/src crates/cgn-mcp/tests
touch crates/cgn-mcp/src/argv.rs
touch crates/cgn-mcp/src/daemon.rs
touch crates/cgn-mcp/src/registry.rs
touch crates/cgn-mcp/src/server.rs
touch crates/cgn-mcp/src/spawn.rs
```
...
Expected: CLI still builds (no actual usage of cgn-mcp yet).

- [ ] **Step 5: Commit**

```bash
git add crates/cgn-mcp Cargo.toml crates/cgn-cli/Cargo.toml
git commit -m "feat(mcp): scaffold cgn-mcp crate
...
### Task 3: Registry — `CgnMcpTool` struct + inventory collect + name derivation

**Files:**
- Modify: `crates/cgn-mcp/src/registry.rs`
- Create: `crates/cgn-mcp/tests/registry_test.rs`

- [ ] **Step 1: Write the failing test**

```rust
// crates/cgn-mcp/tests/registry_test.rs
use cgn_mcp::registry::{derive_subcommand, derive_tool_name};

#[test]
fn derive_tool_name_extracts_last_segment_with_cgn_prefix() {
    assert_eq!(derive_tool_name("cgn_cli::commands::context"), "cgn_context");
    assert_eq!(derive_tool_name("cgn_cli::commands::detect_changes"), "cgn_detect_changes");
    assert_eq!(derive_tool_name("cgn_cli::commands::multi_query"), "cgn_multi_query");
}

#[test]
fn derive_subcommand_returns_last_segment_raw() {
    assert_eq!(derive_subcommand("cgn_cli::commands::context"), "context");
    assert_eq!(derive_subcommand("cgn_cli::commands::detect_changes"), "detect_changes");
}
```
...
```bash
cargo test -p cgn-mcp --test registry_test
```

Expected: FAIL with `unresolved import cgn_mcp::registry::derive_tool_name`.

- [ ] **Step 3: Implement registry.rs**

```rust
// crates/cgn-mcp/src/registry.rs
...
//!
//! All four are filled by the `cgn_register_mcp_tool!` macro (Task 11).
//! At runtime, the MCP server iterates `inventory::iter::<CgnMcpTool>()`
//! and registers each.
...
/// the whole `cgn-cli` Engine type into its public API.
...
pub struct CgnMcpTool {
    ...
    pub handler: fn(Value, &dyn EngineRef) -> Result<Value, CgnError>,
...
/// Strip the leading `cgn_cli::commands::` (or any prefix) and
/// prepend `cgn_`. The last `::` segment IS the subcommand identifier
...
pub fn derive_tool_name(module_path: &str) -> &'static str {
    ...
    Box::leak(format!("cgn_{last}").into_boxed_str())
}
...
```bash
git add crates/cgn-mcp/src/registry.rs crates/cgn-mcp/tests/registry_test.rs
git commit -m "feat(mcp): CgnMcpTool registry + name derivation helpers

inventory::collect! makes CgnMcpTool collectable at link time.
...
**Files:**
- Modify: `crates/cgn-mcp/src/argv.rs`
- Create: `crates/cgn-mcp/tests/argv_test.rs`

- [ ] **Step 1: Write the failing test**

```rust
// crates/cgn-mcp/tests/argv_test.rs
use cgn_mcp::argv::json_to_argv;
use serde_json::json;
...
```bash
cargo test -p cgn-mcp --test argv_test
```

Expected: FAIL with `unresolved import cgn_mcp::argv::json_to_argv`.

- [ ] **Step 3: Implement `json_to_argv`**

```rust
// crates/cgn-mcp/src/argv.rs
//! Convert MCP-side JSON args into the clap CLI flag form cgn
//! subcommands expect.
//!
...
```bash
git add crates/cgn-mcp/src/argv.rs crates/cgn-mcp/tests/argv_test.rs
...
errors on nested objects/arrays (MVP scope — cgn args are flat)."
```
...
4. Add `cgn_register_mcp_tool!` macro line at module bottom
...
**Files:**
- Modify: `crates/cgn-cli/src/commands/context.rs`

- [ ] **Step 1: Read the current shape**

```bash
head -40 crates/cgn-cli/src/commands/context.rs
```
...
Append to the bottom of `crates/cgn-cli/src/commands/context.rs`:
...
mod inner_tests {
    ...
    fn run_inner_returns_structured_value_not_unit() {
        ...
        fn _accepts(_f: fn(ContextArgs, &crate::engine::Engine) -> Result<serde_json::Value, cgn_core::CgnError>) {}
        _accepts(run_inner);
    }
}
```
...
```bash
cargo test -p code-graph-nexus --lib commands::context::inner_tests
```
...
pub fn run_inner(args: ContextArgs, engine: &crate::engine::Engine)
    -> Result<serde_json::Value, cgn_core::CgnError>
{
...
```bash
cargo test -p code-graph-nexus --lib commands::context
```
...
```bash
git add crates/cgn-cli/src/commands/context.rs
...
|---|---|---|---|
| 6 | `crates/cgn-cli/src/commands/impact.rs` | `ImpactArgs` | `refactor(impact): split run into run_inner + emit shell` |
| 7 | `crates/cgn-cli/src/commands/query.rs` | `QueryArgs` | `refactor(query): split run into run_inner + emit shell` |
| 8 | `crates/cgn-cli/src/commands/detect_changes.rs` | `DetectChangesArgs` | `refactor(detect_changes): split run into run_inner + emit shell` |
| 9 | `crates/cgn-cli/src/commands/route_map.rs` | `RouteMapArgs` | `refactor(route_map): split run into run_inner + emit shell` |
| 10a | `crates/cgn-cli/src/commands/rename.rs` | `RenameArgs` | `refactor(rename): split run into run_inner + emit shell` |
| 10b | `crates/cgn-cli/src/commands/shape_check.rs` | `ShapeCheckArgs` | `refactor(shape_check): split run into run_inner + emit shell` |
| 10c | `crates/cgn-cli/src/commands/multi_query.rs` | `MultiQueryArgs` | `refactor(multi_query): split run into run_inner + emit shell` |
...
- [ ] **Step 2: Run + see it fail** — `cargo test -p code-graph-nexus --lib commands::<MODULE>::inner_tests` (e.g. `commands::impact::inner_tests`). Expected: FAIL with `cannot find function run_inner in this scope`.
...
- [ ] **Step 3: Apply the four-piece refactor identically to Task 5**:
  ...
  4. Split the existing `run` function: rename the original body to `run_inner(args: <ARGS>, engine: &crate::engine::Engine) -> Result<serde_json::Value, cgn_core::CgnError>` and change its terminal `crate::output::emit(&value, format)` to return the value (`Ok(value)`). Add a new thin `run` shell exactly like Task 5's:
     ```rust
...
- [ ] **Step 4: Run all of the command's tests** — `cargo test -p code-graph-nexus --lib commands::<MODULE>` AND the command's integration tests (e.g. for `impact.rs` also run `cargo test -p code-graph-nexus --test impact_cmd` if such a file exists; check `crates/cgn-cli/tests/` for matching names). Expected: pass; no regressions vs main.
...
### Task 11: Define `cgn_register_mcp_tool!` macro

**Files:**
- Create: `crates/cgn-mcp/src/macros.rs`
- Modify: `crates/cgn-mcp/src/lib.rs` (add `pub mod macros;`)

- [ ] **Step 1: Write a compile-test fixture**

```rust
// crates/cgn-mcp/tests/macro_test.rs
//! Compile-only smoke test that the macro expands cleanly. Real
//! end-to-end registration is verified in server_e2e (Task 18).

use cgn_mcp::cgn_register_mcp_tool;
use cgn_mcp::registry::EngineRef;
use serde::{Deserialize, Serialize};
use schemars::JsonSchema;
...
cgn_register_mcp_tool!(DummyArgs, run_inner);

#[test]
fn macro_registers_dummy_tool_via_inventory() {
    let found: Vec<&'static cgn_mcp::registry::CgnMcpTool> =
        inventory::iter::<cgn_mcp::registry::CgnMcpTool>().collect();
    let names: Vec<&str> = found.iter().map(|t| t.name).collect();
    assert!(
        names.contains(&"cgn_macro_test"),
        "expected cgn_macro_test in registry; got {:?}", names
    );
}
```
...
```bash
cargo test -p cgn-mcp --test macro_test
```

Expected: FAIL — `cgn_register_mcp_tool` macro doesn't exist.

- [ ] **Step 3: Define the macro**

```rust
// crates/cgn-mcp/src/macros.rs
            ...
            $crate::registry::CgnMcpTool {
                ...
                handler: |raw, engine| {
                    let parsed: $args = ::serde_json::from_value(raw)
                        .map_err(|e| ::cgn_core::CgnError::InvalidArgument(
...
Edit `crates/cgn-mcp/src/lib.rs`:
...
```bash
git add crates/cgn-mcp/src/macros.rs crates/cgn-mcp/src/lib.rs crates/cgn-mcp/tests/macro_test.rs
git commit -m "feat(mcp): cgn_register_mcp_tool! macro + inventory submit
...
**Files:**
- Modify: 8 files — `crates/cgn-cli/src/commands/{context,impact,query,detect_changes,rename,route_map,shape_check,multi_query}.rs`

- [ ] **Step 1: Write the inventory completeness test**

Create `crates/cgn-cli/tests/mcp_tool_registry_test.rs`:
...
fn expected_eight_tools_present() {
    ...
    // cgn-cli library.
    use cgn_mcp::registry::CgnMcpTool;
    let names: std::collections::BTreeSet<&str> =
        inventory::iter::<CgnMcpTool>().map(|t| t.name).collect();
    let expected = [
        "cgn_context",
        "cgn_detect_changes",
        "cgn_impact",
...
```bash
cargo test -p code-graph-nexus --test mcp_tool_registry_test
```
...
```rust
cgn_mcp::cgn_register_mcp_tool!(ContextArgs, run_inner);
```
...
```bash
git add crates/cgn-cli/src/commands/*.rs crates/cgn-cli/tests/mcp_tool_registry_test.rs
git commit -m "feat(cli): register 8 commands as MCP tools via inventory macro

One cgn_register_mcp_tool! line per command file. Tool names derived
...
**Files:**
- Modify: `crates/cgn-mcp/src/spawn.rs`
- Create: `crates/cgn-mcp/tests/spawn_test.rs`

- [ ] **Step 1: Write the failing test**

```rust
// crates/cgn-mcp/tests/spawn_test.rs
...
```bash
cargo test -p cgn-mcp --test spawn_test
```
...
```rust
// crates/cgn-mcp/src/spawn.rs
//! Spawn-mode dispatch. Each tool call → `Command::new(cgn).arg(subcmd).args(argv).output()`.
...
pub fn run_spawn(binary: &Path, subcommand: &str, args: &Value) -> Result<String> {
    ...
    if !output.status.success() {
        return Err(anyhow!(
            "cgn {subcommand} exited with {} — stderr:\n{}",
            output.status,
...
```bash
git add crates/cgn-mcp/src/spawn.rs crates/cgn-mcp/tests/spawn_test.rs
git commit -m "feat(mcp): spawn-mode dispatch — run_spawn(binary, subcommand, args)

Synchronously spawns the cgn binary, captures stdout, surfaces
...
**Files:**
- Modify: `crates/cgn-mcp/src/daemon.rs`
- Create: `crates/cgn-mcp/tests/daemon_test.rs`

- [ ] **Step 1: Write the failing test**

```rust
// crates/cgn-mcp/tests/daemon_test.rs
use cgn_mcp::daemon::needs_remap;
use std::fs;
...
```bash
cargo test -p cgn-mcp --test daemon_test
```
...
```rust
// crates/cgn-mcp/src/daemon.rs
//! Daemon-mode dispatch. Engine mmap'd once at server startup; refreshed
//! via mtime-remap before every dispatch.
//!
//! Why mtime-remap: `cgn analyze` writes graph.bin via atomic
//! write-tmp + rename (see crates/cgn-core/src/registry/io.rs:33).
...
```bash
git add crates/cgn-mcp/src/daemon.rs crates/cgn-mcp/tests/daemon_test.rs
...
cgn analyze and our mmap is now pointing at the unlinked old inode."
```
...
**Files:**
- Modify: `crates/cgn-mcp/src/server.rs`
- Create: `crates/cgn-mcp/tests/server_smoke_test.rs`

- [ ] **Step 1: Write the smoke test**

```rust
// crates/cgn-mcp/tests/server_smoke_test.rs
//! Smoke: build a server, list-tools, expect the inventory contents.

use cgn_mcp::server::{DispatchMode, CgnMcpServer};

#[tokio::test(flavor = "current_thread")]
async fn list_tools_returns_registered_inventory() {
    let server = CgnMcpServer::new(DispatchMode::Spawn).expect("init");
    let tools = server.list_tools();
    let names: Vec<&str> = tools.iter().map(|t| t.name).collect();
    // From this test crate's perspective, the CLI commands aren't
    // linked in; only fixtures/macros submitted by cgn-mcp's
...
```bash
cargo test -p cgn-mcp --test server_smoke_test
```

Expected: FAIL — `CgnMcpServer` not defined.

- [ ] **Step 3: Implement server.rs**

```rust
// crates/cgn-mcp/src/server.rs
//! Stdio JSON-RPC MCP server scaffold. Wraps `rmcp::ServiceExt` and
//! dispatches tool calls via either spawn or daemon mode.

use crate::registry::{EngineRef, CgnMcpTool};
use anyhow::{Context, Result};
...
pub enum DispatchMode {
    /// Default: spawn `cgn <subcommand>` per call.
    Spawn,
...
pub struct DaemonState {
    ...
    // EngineRef so this crate doesn't depend on cgn-cli's
...
pub struct CgnMcpServer {
    ...
    /// Path to the current cgn binary (used by spawn mode).
    self_exe: PathBuf,
}
...
impl CgnMcpServer {
    ...
    pub fn list_tools(&self) -> Vec<&'static CgnMcpTool> {
        inventory::iter::<CgnMcpTool>().collect()
    }
        ...
        match self.mode {
            ...
            DispatchMode::Daemon => {
                ...
                if crate::daemon::needs_remap(&state.engine_path, state.loaded_at)? {
                    ...
                    // see crates/cgn-cli/src/commands/mcp.rs.
                }
...
```bash
git add crates/cgn-mcp/src/server.rs crates/cgn-mcp/tests/server_smoke_test.rs
git commit -m "feat(mcp): CgnMcpServer scaffold with dual-mode dispatch
...
### Task 16: `cgn mcp` subcommand (CLI side)

**Files:**
- Create: `crates/cgn-cli/src/commands/mcp.rs`
...
```rust
// crates/cgn-cli/tests/mcp_subcommand_test.rs
//! Drives `cgn mcp tools` and asserts the output enumerates the 8
//! expected tools.

use std::process::Command;

#[test]
fn cgn_mcp_tools_lists_eight_tools() {
    let bin = env!("CARGO_BIN_EXE_cgn");
    let out = Command::new(bin).args(["mcp", "tools"]).output().expect("spawn");
    assert!(out.status.success(), "stderr: {}", String::from_utf8_lossy(&out.stderr));
    let stdout = String::from_utf8(out.stdout).unwrap();
    for expected in [
        "cgn_context", "cgn_impact", "cgn_query", "cgn_detect_changes",
        "cgn_rename", "cgn_route_map", "cgn_shape_check", "cgn_multi_query",
    ] {
...
```bash
cargo test -p code-graph-nexus --test mcp_subcommand_test
```
...
```rust
// crates/cgn-cli/src/commands/mcp.rs
...
pub enum McpAction {
    ...
    Serve {
        ...
        /// the same resolution cgn commands use).
        #[arg(long)]
        graph: Option<String>,
    },
...
pub fn run(args: McpArgs) -> Result<(), CgnError> {
    match args.action {
        McpAction::Tools => {
            let server = CgnMcpServer::new(DispatchMode::Spawn)
                .map_err(|e| CgnError::Output(format!("server init: {e}")))?;
        ...
        McpAction::Serve { daemon, graph: _ } => {
            ...
            let mut server = CgnMcpServer::new(mode)
                .map_err(|e| CgnError::Output(format!("server init: {e}")))?;
            if daemon {
            ...
            rt.block_on(async move {
                cgn_mcp::server::serve_stdio(server).await
...
Also add to `crates/cgn-mcp/src/server.rs` (append to the existing module):
...
pub async fn serve_stdio(server: CgnMcpServer) -> anyhow::Result<()> {
    ...
    // Bind our CgnMcpServer to that trait inline.
    impl rmcp::ServerHandler for CgnMcpServer {
        ...
        // turn each CgnMcpTool into rmcp::Tool { name, description,
...
`CgnMcpTool` into an rmcp tool registration; delegate `call_tool` to
`CgnMcpServer::call_tool`) is the only thing that's fixed.
...
Edit `crates/cgn-cli/src/main.rs`, add to the `Commands` enum:
...
git add \
  ...
  crates/cgn-mcp/src/server.rs \
  crates/cgn-cli/tests/mcp_subcommand_test.rs
git commit -m "feat(cli): cgn mcp serve / tools subcommand

cgn mcp tools enumerates the 8 in-scope registered tools (test
invariant). cgn mcp serve runs the stdio JSON-RPC server in spawn
...
### Task 17: End-to-end test — manual MCP client against `cgn mcp serve`

**Files:**
- Create: `crates/cgn-mcp/tests/server_e2e.rs`

- [ ] **Step 1: Write the e2e test**

```rust
// crates/cgn-mcp/tests/server_e2e.rs
//! End-to-end: pipe a real MCP JSON-RPC sequence into `cgn mcp serve`'s
//! stdin, read the JSON-RPC response, assert it lists the 8 tools and
//! a tools/call against cgn_context returns plausible output.

use std::io::Write;
use std::process::{Command, Stdio};

fn cgn_bin() -> String {
    env!("CARGO_BIN_EXE_cgn").to_string()
}
...
```bash
cargo test -p cgn-mcp --test server_e2e
```

Expected: PASS — `cgn mcp serve` honors MCP protocol and lists 8 tools.
...
```bash
git add crates/cgn-mcp/tests/server_e2e.rs
git commit -m "test(mcp): e2e — pipe JSON-RPC into cgn mcp serve

Spawns the real cgn binary, sends initialize + tools/list, asserts
...
cgn mcp serve --daemon    # currently returns NotImplementedError, pending TUI install handler
```
...
```markdown
**Status**: Spec landed; subproject A (cgn-mcp crate, single-binary
`cgn mcp serve`, spawn-mode dispatch) implemented and shipped. Daemon-mode
...
Documents cgn mcp serve / tools subcommands and manual host config
...
|---|---|
| Single-binary `cgn mcp serve` | Task 16 |
| `cgn mcp serve --daemon` flag | Task 16 (parses); Task 14 (mtime-remap); daemon-dispatch wiring deferred to subproject C |
...
| `cgn_register_mcp_tool!` macro | Task 11 |
| Per-command self-registration | Task 12 |
| Tool set (auto-discovered 8) | Task 12 (registration); Task 16 (`cgn mcp tools` enumerates) |
