...
**Audience:** A Sonnet subagent dispatched to add one new language to `gnx-analyzer`.
...
Every language has the same shape in `crates/gnx-analyzer/src/<lang>/`:
...
- `crates/gnx-analyzer/src/c/parser.rs` — 127 lines, **smallest and cleanest**, use this as your starting template
- `crates/gnx-analyzer/src/rust/parser.rs` — 183 lines, similar simplicity
- **Do NOT** template from `crates/gnx-analyzer/src/typescript/parser.rs` — it has a route-dedup loop only TS needs
...
`gnx_core::graph::NodeKind` variants you'll use:
...
### 4.2 `crates/gnx-analyzer/src/lib.rs`
...
### 4.3 `crates/gnx-cli/src/commands/analyze.rs`
...
cargo build -p gnx-analyzer 2>&1 | tail -20
cargo build -p gnx-cli 2>&1 | tail -20

# 2. Indexing must succeed (produce a non-empty graph.bin)
target/debug/gnx-cli analyze --repo tests/parity/fixtures/<lang>/sample_project
ls -la tests/parity/fixtures/<lang>/sample_project/.gitnexus-rs/graph.bin

# 3. One symbol must resolve
target/debug/gnx-cli context \
...
1. **You must `cargo build -p gnx-analyzer` and `cargo build -p gnx-cli` and both must succeed before declaring done.** Report the last 20 lines of each build's output in your response.
2. **You must run `gnx-cli analyze` on your fixture and confirm `.gitnexus-rs/graph.bin` exists.** Report `ls -la` of the output.
3. **You must run `gnx-cli context --name <some_symbol>` on your fixture and get a non-error response.** Report the JSON output.
