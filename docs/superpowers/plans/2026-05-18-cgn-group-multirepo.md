# `cgn group` Multi-repo Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Implement the `cgn group <verb>` noun-first namespace
(`sync / status / contracts / impact / search / find / coverage`) per
`docs/specs/2026-05-18-cgn-group-multirepo-design.md`, including the
...
crate (no new crate). Storage: `~/.cgn/groups/<name>/{contracts.rkyv,
...
(already wired via `cgn-cli/src/search.rs`), tree-sitter
...
**Files:**
- Create: `crates/cgn-cli/src/commands/group/mod.rs`
- Create: `crates/cgn-cli/src/commands/group/types.rs`
- Create: `crates/cgn-cli/src/commands/group/storage.rs`
- Modify: `crates/cgn-cli/src/commands/mod.rs` (register `pub mod group;`)
- Test: `crates/cgn-cli/tests/group_storage.rs`

- [ ] **Step 1: Add module declaration**

Edit `crates/cgn-cli/src/commands/mod.rs`, add alongside
other `pub mod`s:
...
```rust
//! `cgn group <verb>` — multi-repo workflow surface.
//!
...
Create `crates/cgn-cli/tests/group_storage.rs`:
...
```
cargo test -p code-graph-nexus --test group_storage
```
...
pub fn write_meta(group_dir: &Path, meta: &GroupMeta) -> io::Result<()> {
    fs::create_dir_all(group_dir)?;
    cgn_core::registry::io::atomic_write_json(&group_dir.join(META_FILE), meta)
}
...
```
grep -n "pub fn atomic_write_json" crates/cgn-core/src/registry/io.rs
```

If not `pub`, promote it (single-line edit) and add a one-line WHY
comment: `// pub: reused by cgn-cli::commands::group::storage`.
...
```
git add crates/cgn-cli/src/commands/group/ \
        crates/cgn-cli/src/commands/mod.rs \
        crates/cgn-cli/tests/group_storage.rs
[ -n "$(git status --porcelain crates/cgn-core/src/registry/io.rs)" ] && \
  git add crates/cgn-core/src/registry/io.rs
...
atomic-rename. Reuses atomic_write_json from cgn-core.
...
**Files:**
- Modify: `crates/cgn-core/src/config.rs`
- Test: `crates/cgn-core/tests/config_group.rs`

- [ ] **Step 1: Write the failing test**

Create `crates/cgn-core/tests/config_group.rs`:
...
```
cargo test -p cgn-core --test config_group
```

Expected: compile error — `Config` has no `group` field.

- [ ] **Step 3: Extend `Config` in `crates/cgn-core/src/config.rs`**
...
- [ ] **Step 5: Verify `cgn admin config` displays new fields**

```
cargo run -p code-graph-nexus --bin cgn -- admin config show 2>&1 | grep -A2 "\[group\]"
```
...
```
git add crates/cgn-core/src/config.rs \
        crates/cgn-core/tests/config_group.rs \
        crates/cgn-cli/src/commands/admin/config.rs  # only if touched in step 5
git commit -m "$(cat <<'EOF'
feat(config): GroupConfig section with BM25 / cross-depth / timeout knobs

All thresholds default-on, override via ~/.cgn/config.toml. No
...
**Files:**
- Create: `crates/cgn-cli/src/commands/group/extractors/mod.rs`
- Modify: `crates/cgn-cli/src/commands/group/mod.rs` (add `pub mod extractors;`)
- Test: `crates/cgn-cli/tests/group_extractor_registry.rs`

- [ ] **Step 1: Write the failing test**

Create `crates/cgn-cli/tests/group_extractor_registry.rs`:
...
```
cargo test -p code-graph-nexus --test group_extractor_registry
```
...
```
git add crates/cgn-cli/src/commands/group/ \
        crates/cgn-cli/tests/group_extractor_registry.rs
git commit -m "$(cat <<'EOF'
...
**Files:**
- Create: `crates/cgn-cli/src/commands/group/extractors/http_go.rs`
- Modify: `crates/cgn-cli/src/commands/group/extractors/mod.rs` (wire it)
- Create: `crates/cgn-cli/tests/fixtures/group/go/http_server.go`
- Test: `crates/cgn-cli/tests/group_extract_go_http.rs`
...
`crates/cgn-cli/tests/fixtures/group/go/http_server.go`:
...
`crates/cgn-cli/tests/group_extract_go_http.rs`:
...
```
cargo test -p code-graph-nexus --test group_extract_go_http
```
...
```
grep "tree-sitter-go" crates/cgn-cli/Cargo.toml
```
...
```
cargo clippy -p code-graph-nexus --tests -- -D warnings
git add crates/cgn-cli/src/commands/group/extractors/ \
        crates/cgn-cli/tests/group_extract_go_http.rs \
        crates/cgn-cli/tests/fixtures/group/go/
[ -n "$(git status --porcelain crates/cgn-cli/Cargo.toml)" ] && \
  git add crates/cgn-cli/Cargo.toml
git commit -m "$(cat <<'EOF'
...
**Files:**
- Create: `crates/cgn-cli/src/commands/group/extractors/http_python.rs`
- Create: `crates/cgn-cli/src/commands/group/extractors/http_node.rs`
- Create: `crates/cgn-cli/src/commands/group/extractors/http_java.rs`
- Create: `crates/cgn-cli/src/commands/group/extractors/http_rust.rs`
- Modify: `crates/cgn-cli/src/commands/group/extractors/mod.rs`
- Create: `crates/cgn-cli/tests/fixtures/group/{python,node,java,rust}/http_server.{py,ts,java,rs}`
- Test: `crates/cgn-cli/tests/group_extract_{python,node,java,rust}_http.rs`
...
```
cargo test -p code-graph-nexus --test group_extract_<lang>_http
```
...
```
cargo test -p code-graph-nexus --test 'group_extract_*_http'
```
...
```
git add crates/cgn-cli/src/commands/group/extractors/http_*.rs \
        crates/cgn-cli/src/commands/group/extractors/mod.rs \
        crates/cgn-cli/tests/group_extract_*_http.rs \
        crates/cgn-cli/tests/fixtures/group/
git commit -m "$(cat <<'EOF'
...
**Files:**
- Create: `crates/cgn-cli/src/commands/group/extractors/grpc_go.rs`
- Create: `crates/cgn-cli/tests/fixtures/group/go/grpc_server.go`
- Test: `crates/cgn-cli/tests/group_extract_go_grpc.rs`
- Modify: `crates/cgn-cli/src/commands/group/extractors/mod.rs`
...
```
cargo test -p code-graph-nexus --test group_extract_go_grpc
```
...
cargo test -p code-graph-nexus --test group_extract_go_grpc
cargo clippy -p code-graph-nexus --tests -- -D warnings
git add crates/cgn-cli/src/commands/group/extractors/grpc_go.rs \
        crates/cgn-cli/src/commands/group/extractors/mod.rs \
        crates/cgn-cli/tests/group_extract_go_grpc.rs \
        crates/cgn-cli/tests/fixtures/group/go/grpc_server.go
git commit -m "$(cat <<'EOF'
...
```
cargo test -p code-graph-nexus --test 'group_extract_*_grpc'
```
...
```
cargo clippy -p code-graph-nexus --tests -- -D warnings
git add crates/cgn-cli/src/commands/group/extractors/grpc_*.rs \
        crates/cgn-cli/src/commands/group/extractors/mod.rs \
        crates/cgn-cli/tests/group_extract_*_grpc.rs \
        crates/cgn-cli/tests/fixtures/group/
git commit -m "$(cat <<'EOF'
...
Reuses `TantivyEngine` from `cgn-cli/src/search.rs`. Per-group
Tantivy index lives at `~/.cgn/groups/<name>/contracts_index/`.

**Files:**
- Create: `crates/cgn-cli/src/commands/group/matching.rs`
- Modify: `crates/cgn-cli/src/commands/group/mod.rs` (add `pub mod matching;`)
- Test: `crates/cgn-cli/tests/group_matching.rs`
...
```
cargo test -p code-graph-nexus --test group_matching
```
...
fn build_bm25_index(index_dir: &Path, contracts: &[&StoredContract]) -> io::Result<()> {
    ...
    // cgn-cli/src/search.rs but with a contract-only doc shape.
    use tantivy::doc;
...
- [ ] **Step 4: Verify Tantivy is in `cgn-cli`'s deps**

```
grep tantivy crates/cgn-cli/Cargo.toml
```
...
```
cargo clippy -p code-graph-nexus --tests -- -D warnings
git add crates/cgn-cli/src/commands/group/matching.rs \
        crates/cgn-cli/src/commands/group/mod.rs \
        crates/cgn-cli/tests/group_matching.rs
git commit -m "$(cat <<'EOF'
...
### Task 9: `cgn group sync` command + wiring

**Files:**
- Create: `crates/cgn-cli/src/commands/group/sync.rs`
- Modify: `crates/cgn-cli/src/commands/group/mod.rs` — add
  `pub mod sync;` + a `GroupCommands` enum and a `pub fn run(cmd) ->
  Result<(), CgnError>` dispatcher
- Modify: `crates/cgn-cli/src/main.rs` — register the
  top-level `Group` subcommand
- Test: `crates/cgn-cli/tests/group_sync.rs`
...
/// search / find / coverage). See `cgn group --help`.
Group {
    #[command(subcommand)]
...
`crates/cgn-cli/tests/group_sync.rs`:

```rust
//! End-to-end: 2-repo fixture (Go provider + Python consumer of /api/users),
//! cgn group sync writes contracts.rkyv + meta.json, exact-matches across them.

use assert_cmd::Command;
use cgn_cli::commands::group::storage::{read_contracts, read_meta};
use std::fs;
use tempfile::TempDir;
...
"#);
    Command::cargo_bin("cgn").unwrap()
        .env("CGN_HOME", home.path())
...
```
cargo test -p code-graph-nexus --test group_sync
```
...
```rust
//! `cgn group sync <name>` — extract contracts across group members,
...
use clap::Args;
use cgn_core::config::Config;
use cgn_core::registry::path::resolve_home_cgn;
...
pub fn run(args: SyncArgs) -> Result<(), CgnError> {
    ...
    let entry = registry
        ...
        .ok_or_else(|| CgnError::other(format!("group not found: {}", args.name)))?;
...
```
cargo test -p code-graph-nexus --test group_sync -- --nocapture
```
...
```
cargo clippy -p code-graph-nexus --tests -- -D warnings
git add crates/cgn-cli/src/commands/group/sync.rs \
        crates/cgn-cli/src/commands/group/mod.rs \
        crates/cgn-cli/src/main.rs \
        crates/cgn-cli/tests/group_sync.rs
git commit -m "$(cat <<'EOF'
feat(group): cgn group sync — extract contracts + write registry
...
### Task 10: `cgn group status`

**Files:**
- Create: `crates/cgn-cli/src/commands/group/status.rs`
- Modify: `commands/group/mod.rs` — register `Status(status::StatusArgs)`
- Test: `crates/cgn-cli/tests/group_status.rs`
...
fn status_reports_stale_when_meta_commit_differs_from_head() {
    ...
    let out = Command::cargo_bin("cgn").unwrap()
        .env("CGN_HOME", home.path())
...
```
cargo test -p code-graph-nexus --test group_status
```

- [ ] **Step 5: Commit**

```
git add crates/cgn-cli/src/commands/group/status.rs \
        crates/cgn-cli/src/commands/group/mod.rs \
        crates/cgn-cli/tests/group_status.rs
git commit -m "feat(group): cgn group status — index + contracts staleness
...
### Task 11: `cgn group contracts`

**Files:**
- Create: `crates/cgn-cli/src/commands/group/contracts.rs`
- Modify: `commands/group/mod.rs`
- Test: `crates/cgn-cli/tests/group_contracts.rs`

- [ ] **Step 1: Write failing test**

```rust
use cgn_cli::commands::group::storage::{group_dir, write_contracts};
use cgn_cli::commands::group::types::*;
use assert_cmd::Command;
use tempfile::TempDir;
...
```
git add crates/cgn-cli/src/commands/group/contracts.rs \
        crates/cgn-cli/src/commands/group/mod.rs \
        crates/cgn-cli/tests/group_contracts.rs
git commit -m "feat(group): cgn group contracts — inspect registry with filters
...
- Test: `crates/cgn-cli/tests/group_impact.rs`

- [ ] **Step 1: Survey existing impact entry**

```
grep -n "pub fn run\|fn run_for\|fn compute_impact" crates/cgn-cli/src/commands/impact.rs
```
...
fn impact_cross_repo_emits_consumer_when_provider_changed() {
    let home = TempDir::new().unwrap();
    // … set up two-repo fixture with exact cross-link, run `cgn group sync demo`,
    // then `cgn group impact demo --target create_user --repo backend --direction downstream`
    let out = Command::cargo_bin("cgn").unwrap()
        .env("CGN_HOME", home.path())
...
```
cargo test -p code-graph-nexus --test group_impact
cargo clippy -p code-graph-nexus --tests -- -D warnings
git add crates/cgn-cli/src/commands/group/impact.rs \
        crates/cgn-cli/src/commands/group/mod.rs \
        crates/cgn-cli/src/commands/impact.rs
git commit -m "feat(group): cgn group impact — local impact + cross-repo fan-out
...
### Task 13: `cgn group search` + `find` + `coverage` (thin wrappers)

Bundled — each is ~40 LOC. Same pattern: resolve members, fan out
existing engine via rayon, optionally merge.

**Files:**
- Create: `crates/cgn-cli/src/commands/group/search.rs`
- Create: `crates/cgn-cli/src/commands/group/find.rs`
- Create: `crates/cgn-cli/src/commands/group/coverage.rs`
- Modify: `commands/group/mod.rs`
- Test: `crates/cgn-cli/tests/group_search.rs`
- Test: `crates/cgn-cli/tests/group_find.rs`
- Test: `crates/cgn-cli/tests/group_coverage.rs`

- [ ] **Step 1: Write failing tests** — one per command. Each test:

  1. Creates a 2-repo fixture (Go + Python) and indexes both via
     `cgn admin index`.
  2. Forms a group via `cgn admin group add demo …`.
  3. Runs `cgn group <verb> demo …` and asserts the JSON output has
...
pub fn run(args: SearchArgs) -> Result<(), CgnError> {
    let home = resolve_home_cgn()?;
    let registry = Registry::open(&home)?;
    let entry = registry.find_group(&args.name)
        .ok_or_else(|| CgnError::other(format!("group not found: {}", args.name)))?;
...
```
cargo test -p code-graph-nexus --test 'group_{search,find,coverage}'
cargo clippy -p code-graph-nexus --tests -- -D warnings
git add crates/cgn-cli/src/commands/group/{search,find,coverage}.rs \
        crates/cgn-cli/src/commands/group/mod.rs \
        crates/cgn-cli/src/commands/{search,find,coverage}.rs \
        crates/cgn-cli/tests/group_search.rs \
        crates/cgn-cli/tests/group_find.rs \
        crates/cgn-cli/tests/group_coverage.rs
git commit -m "feat(group): cgn group search / find / coverage thin wrappers
...
**Files:**
- Modify: `crates/cgn-cli/src/repo_selector.rs` — add an
  enum variant signalling "@group reached top-level dispatcher"
- Modify: `crates/cgn-cli/src/commands/search.rs`
- Modify: `crates/cgn-cli/src/commands/find.rs`
- Modify: `crates/cgn-cli/src/commands/contracts.rs`
- Modify: `crates/cgn-cli/src/commands/coverage.rs`
- Delete: `crates/cgn-cli/tests/search_multi_repo.rs` — replaced by `tests/group_search.rs`
- Modify: `crates/cgn-cli/tests/search_cmd.rs` — remove
  `search_multi_repo_at_*` tests, keep `@all` tests
- Test: `crates/cgn-cli/tests/group_at_repo_top_level_errors.rs`
...
fn search_at_group_returns_error_with_hint() {
    ...
    assert!(s.contains("use `cgn group search`"), "missing hint; got: {s}");
}

#[test]
fn contracts_at_group_returns_error_with_hint() {
    /* same shape; expects "use `cgn group contracts`" */
}

#[test]
fn find_at_group_returns_error_with_hint() {
    /* same shape; expects "use `cgn group find`" */
}

#[test]
fn coverage_at_group_returns_error_with_hint() {
    /* same shape; expects "use `cgn group coverage`" */
}
...
```
cargo test -p code-graph-nexus --test group_at_repo_top_level_errors
```
...
```rust
#[error("`@{0}` cannot be used at the top level — use `cgn group {hint}` instead")]
...
```
git rm crates/cgn-cli/tests/search_multi_repo.rs
```

In `crates/cgn-cli/tests/search_cmd.rs`, delete:
...
hint pointing at `cgn group <verb>`. @all and single-repo selectors
...
**Files:**
- Modify: `crates/cgn-mcp/src/handlers/` — every handler
  whose schema declares `repo: string` and that calls into the same
  selector path. Inventory via:
  ```
  grep -rn "@.*group\|Atom::Group\|repo_selector" crates/cgn-mcp/src/
  ```
- Test: `crates/cgn-mcp/tests/mcp_group_rejected.rs` (or
  extend existing handler tests)

- [ ] **Step 1: Inventory MCP handlers**

```
grep -rn "fn.*repo\|RepoSelector\|repo_selector::resolve" crates/cgn-mcp/src/
```
...
async fn mcp_find_at_group_returns_hint_error() {
    let resp = call_mcp_tool("mcp__cgn__find", serde_json::json!({
        "repo": "@demo",
        "pattern": "x",
    })).await;
    let err = resp.as_object().unwrap().get("error").unwrap().as_str().unwrap();
    assert!(err.contains("use `cgn group find`"), "got: {err}");
}
```
...
```
cargo test -p cgn-mcp
```

- [ ] **Step 5: Clippy + commit**

```
cargo clippy -p cgn-mcp -- -D warnings
git add -A
git commit -m "$(cat <<'EOF'
...
New mcp__cgn_group_* tools deferred to a follow-up PR — for now,
agents shell out to cgn group <verb>.
...
**Files:**
- Modify: `docs/skills/cgn.md`
- Modify: `README.md` (Language Matrix table)
- Modify: `docs/specs/2026-05-18-cgn-group-multirepo-design.md` —
  flip Status from `Draft` to `Shipped`, add a "Shipped" date.

- [ ] **Step 1: Update `docs/skills/cgn.md`**
...
In `docs/specs/2026-05-18-cgn-group-multirepo-design.md`, change:
...
```
cargo test -p code-graph-nexus --tests
cargo test -p cgn-analyzer --tests
cargo test -p cgn-core --tests
cargo test -p cgn-mcp --tests
...
- [ ] **Step 5: Bench against `scripts/benchmark_cgn.py`**

```
python scripts/benchmark_cgn.py
```
...
```
git add docs/skills/cgn.md README.md docs/specs/2026-05-18-cgn-group-multirepo-design.md
git commit -m "$(cat <<'EOF'
docs(group): cgn.md + README matrix + flip spec status to shipped
...
Brings the `cgn group <verb>` noun-first namespace into the CLI per
[docs/specs/2026-05-18-cgn-group-multirepo-design.md](docs/specs/2026-05-18-cgn-group-multirepo-design.md):
...
- [ ] `cargo test -p code-graph-nexus --tests` — all green
- [ ] `cargo test -p cgn-analyzer --tests` — all green
- [ ] `cargo test -p cgn-core --tests` — all green
- [ ] `cargo test -p cgn-mcp --tests` — all green
- [ ] `cargo clippy --workspace --tests -- -D warnings` — clean
- [ ] `python scripts/benchmark_cgn.py` — `group status` < 200 ms,
      `group impact` < 200 ms for direct=20 / group=5
- [ ] Manual: form a 2-repo group, `cgn group sync demo`,
      `cgn group contracts demo`, verify TOON output shape matches spec
