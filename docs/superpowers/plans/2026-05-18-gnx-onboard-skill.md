# gnx-onboard SKILL Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Build the gnx-onboard SKILL pack — a personalized installation + configuration wizard for `graph-nexus` distributed as a layered markdown SKILL — entirely inside the existing `gitnexus-rs` repo, ready for the 4 distribution outlets defined in the spec.

**Architecture:** Three-layer SKILL pack under `docs/skills/gnx-onboard/` (SKILL.md + guides/ + _shared/), bash tooling under `tools/`, tests + smoke playbook under `tests/skill/`, CI workflows under `.github/workflows/`. The pack reuses the existing `gnx` CLI release cycle (single repo, zero drift). Implementation TDDs all bash tools (each tool ships with a `tests/skill/test-<tool>.sh` integration test); markdown content is validated by the lint tool + aggregator round-trip.

**Tech Stack:** bash 5.x, jq, awk, sed, diff, GitHub Actions, markdown. No language runtime required beyond standard *nix tools. `gnx` (built from this same workspace) for CLI-ref generation.
...
```
docs/skills/gnx-onboard/                            ← Layer 1 + 2 + 3 SKILL pack
...
└── _shared/
    ...
    │   └── 0.1.5/                                   ← (Task 21) generated; one .md per gnx subcmd
    └── refs/
...
**Files:**
- Create: `docs/skills/gnx-onboard/.gitkeep`
- Create: `docs/skills/gnx-onboard/guides/.gitkeep`
- Create: `docs/skills/gnx-onboard/_shared/cli/.gitkeep`
- Create: `docs/skills/gnx-onboard/_shared/refs/.gitkeep`
...
```bash
mkdir -p docs/skills/gnx-onboard/guides
mkdir -p docs/skills/gnx-onboard/_shared/cli
mkdir -p docs/skills/gnx-onboard/_shared/refs
mkdir -p tools
mkdir -p tests/skill
```
...
```bash
touch docs/skills/gnx-onboard/.gitkeep \
      docs/skills/gnx-onboard/guides/.gitkeep \
      docs/skills/gnx-onboard/_shared/cli/.gitkeep \
      docs/skills/gnx-onboard/_shared/refs/.gitkeep \
      tools/.gitkeep \
      tests/skill/.gitkeep
```
...
Run: `find docs/skills/gnx-onboard tools tests/skill -type d`
Expected: shows all 7 directories.

- [ ] **Step 4: Commit**

```bash
git add docs/skills/gnx-onboard tools/.gitkeep tests/skill/.gitkeep
...
cat > "$tmp/docs/skills/gnx-onboard/guides/99-leftover.md" <<'EOF'
# orphan
EOF
assert_exit 1 bash "$LINT" "$tmp/docs/skills/gnx-onboard"

# Remove orphan → pass
rm "$tmp/docs/skills/gnx-onboard/guides/99-leftover.md"
assert_exit 0 bash "$LINT" "$tmp/docs/skills/gnx-onboard"
...
bash "$AGG" "$tmp/docs/skills/gnx-onboard" > "$tmp/ONBOARDING.md"
...
assert_no_grep '^name: gnx-onboard' "$tmp/ONBOARDING.md"
...
- [ ] **Step 1: Write the failing test (mock gnx via shell stub)**
...
**Files:**
- Create: `docs/skills/gnx-onboard/_shared/refs/persona-inference.md`
...
| Chat contains "cargo" or "rust" or "Rust workspace" | install_pref = cargo-binstall | Recommend `cargo binstall graph-nexus` |
...
```bash
git add docs/skills/gnx-onboard/_shared/refs/persona-inference.md
...
**Files:**
- Create: `docs/skills/gnx-onboard/_shared/refs/env-detect.md`
...
```bash
git add docs/skills/gnx-onboard/_shared/refs/env-detect.md
...
**Files:**
- Create: `docs/skills/gnx-onboard/_shared/refs/recommendation-templates.md`
...
- Run `gnx group find <group> "<symbol>" --merge rrf` to do cross-repo BM25 search with RRF fusion.
- Run `gnx group contracts <group>` to inventory routes / queue / RPC contracts across the group.
- Run `gnx group impact <group> --baseline origin/main` to see the full blast radius of a multi-repo change before merging.

### scope_pref = single-repo

- Run `gnx find "<symbol>" --repo .` to look up the canonical definition.
- Run `gnx impact <symbol> --direction upstream --repo .` to see callers.
- Run `gnx routes --repo .` to list HTTP routes mapped to handlers.

## By ide_pref

### ide_pref = claude-code

- Open a Claude Code session in `{repo_name}` and ask "summarize the auth module"; the gnx MCP tools should appear automatically.
- Type `/gnx` in Claude Code to see the cheatsheet skill loaded.
...
- Zed's assistant panel will list `gnx_*` tools once the config is reloaded.
...
- `cargo binstall --self-update` keeps cargo-binstall current so future gnx upgrades stay fast.
- Run `gnx --version` periodically; cargo-binstall does NOT auto-upgrade gnx itself.

### install_pref = brew

- `brew upgrade graph-nexus` will pull the latest tagged release.

### install_pref = github-release-tarball

- Bookmark `gh release view --repo <owner>/graph-nexus` to spot new releases.

## Universal (always offer 1)

- Bookmark this summary file (`~/.gnx/onboarding-summary.md`) — a future agent session can read it to know what was set up.
- Run `gnx coverage --repo @all --detailed` to inspect registry health.
- Run `gnx admin mcp tools` to list the MCP tools currently exposed.
...
```bash
git add docs/skills/gnx-onboard/_shared/refs/recommendation-templates.md
...
**Files:**
- Create: `docs/skills/gnx-onboard/SKILL.md`
...
```bash
bash tools/lint-skill.sh docs/skills/gnx-onboard
```
...
```bash
git add docs/skills/gnx-onboard/SKILL.md
...
**Files:**
- Create: `docs/skills/gnx-onboard/guides/01-install.md`
...
```bash
git add docs/skills/gnx-onboard/guides/01-install.md
...
**Files:**
- Create: `docs/skills/gnx-onboard/guides/02-first-index.md`
...
```bash
git add docs/skills/gnx-onboard/guides/02-first-index.md
...
**Files:**
- Create: `docs/skills/gnx-onboard/guides/03-group.md`
...
```bash
git add docs/skills/gnx-onboard/guides/03-group.md
...
**Files:**
- Create: `docs/skills/gnx-onboard/guides/04-mcp.md`
...
```bash
git add docs/skills/gnx-onboard/guides/04-mcp.md
...
**Files:**
- Create: `docs/skills/gnx-onboard/guides/05-summary.md`
...
```bash
git add docs/skills/gnx-onboard/guides/05-summary.md
...
bash tools/lint-skill.sh docs/skills/gnx-onboard
```
Expected: `lint OK: docs/skills/gnx-onboard`
...
## Task 21: Generate Layer 3 CLI cards from local gnx

**Files:**
- Create: `docs/skills/gnx-onboard/_shared/cli/<gnx-version>/*.md`
- Create: `docs/skills/gnx-onboard/_shared/cli/manifest.json`

- [ ] **Step 1: Build gnx locally**

```bash
cargo build --release --bin gnx
./target/release/gnx --version
```
Expected: e.g. `gnx 0.1.5`
...
bash tools/gen-cli-ref.sh \
    ./target/release/gnx \
    docs/skills/gnx-onboard/_shared/cli
```
Expected: `gen-cli-ref: wrote docs/skills/gnx-onboard/_shared/cli/0.1.5/ + manifest`

- [ ] **Step 3: Inspect output**

```bash
ls docs/skills/gnx-onboard/_shared/cli/0.1.5/
cat docs/skills/gnx-onboard/_shared/cli/manifest.json
```
...
```bash
git add docs/skills/gnx-onboard/_shared/cli/
git commit -m "feat(skill): generate CLI reference cards for gnx 0.1.5"
```
...
**Files:**
- Create: `docs/skills/gnx-onboard/ONBOARDING.md`

- [ ] **Step 1: Run the aggregator**

```bash
bash tools/aggregate.sh docs/skills/gnx-onboard \
    > docs/skills/gnx-onboard/ONBOARDING.md
```

- [ ] **Step 2: Sanity-check the output**

```bash
wc -l docs/skills/gnx-onboard/ONBOARDING.md
# Expect 500–800 lines (SKILL.md ~80 + 5 guides at 100–150 each)

grep -c '<!-- guide:' docs/skills/gnx-onboard/ONBOARDING.md
# Expect 5

head -3 docs/skills/gnx-onboard/ONBOARDING.md
...
```bash
git add docs/skills/gnx-onboard/ONBOARDING.md
...
jobs:
  regen:
    ...
    steps:
      ...
      - name: Build gnx
        run: cargo build --release --bin gnx
...
```bash
git add README.md
git commit -m "docs(readme): document 4 distribution paths for gnx-onboard SKILL"
```
...
```bash
find docs/skills/gnx-onboard tools tests/skill .github/workflows \
...
```bash
grep -rE 'TODO|TBD|FIXME|XXX' docs/skills/gnx-onboard tools tests/skill .github/workflows/skill-*.yml
```
Expected: no output.
...
gh pr create --title "feat(skill): gnx-onboard SKILL pack — 3 layers + 4 distribution outlets" \
    --body "$(cat <<'EOF'
## Summary

Implementation of the gnx-onboard SKILL pack per spec
`docs/superpowers/specs/2026-05-18-gnx-onboard-skill-design.md`.

- Layered structure under `docs/skills/gnx-onboard/`: SKILL.md (Layer 1)
...
- `config_inventory` is the same in-memory structure across all phases.
  ...
  ```yaml
  config_inventory:
    install_probe: { os, arch, has_cargo_binstall, has_brew, has_curl, gnx_already_installed }
...
- File-naming convention used throughout:
  - SKILL pack: `docs/skills/gnx-onboard/`
