...
| **Public version string only** | `ecp --version` reveals only the binary's own self-reported version, which is the same as any public release tag |
...
`ecp.{installed,version,registry_exists}`.
...
for tool in cargo cargo-binstall brew curl wget ecp; do
    ( command -v "$tool" >/dev/null 2>&1 && echo true || echo false ) > "$tmp/has_$tool" &
done

( command -v ecp >/dev/null 2>&1 && ecp --version 2>/dev/null | awk '{print $NF}' || echo "" ) > "$tmp/ecp_ver" &
...
( test -f "$HOME/.ecp/registry.json" && echo true || echo false )                > "$tmp/ecp_registry" &
...
jq -n \
  ...
  --argjson g_reg      "$(cat "$tmp/ecp_registry")" \
  '{
     os:    ($os   | rtrimstr("\n")),
     arch:  ($arch | rtrimstr("\n")),
     installers: {
...
| install | binstall fails to fetch tarball | (1) no prebuilt for this target triple → fallback to source build (`cargo install egent-code-plexus`); (2) network / proxy; (3) GitHub release not yet propagated |
| install | `brew install` fails with "no such formula" | tap not added — suggest `brew tap <author>/ecp` |
| install | `curl` of GitHub release returns 404 | version tag mismatch — confirm latest release tag on `gh release list` |
| first-index | `ecp admin index ... → not a git repo` | wrong path / no `.git` directory at repo root |
| first-index | index runs >3 min | large repo / vendored deps not ignored — recommend a `.ecpignore` |
| first-index | `permission denied` writing to `~/.ecp` | recipient's HOME not writable (rare; container env) — suggest `ECP_HOME=$PWD/.ecp` env override |
| group | `ecp admin group add ... → repo not in registry` | repo not yet indexed — re-run phase 02 for that path first |
