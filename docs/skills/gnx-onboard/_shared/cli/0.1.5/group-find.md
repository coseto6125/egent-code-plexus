...
Usage: gnx group find [OPTIONS] <NAME> [PATTERN]
...
Options:
      --merge <MERGE>
          ...
          Possible values:
          - none: Per-repo bucketed concat — every member's hits emitted under its own header. Matches the single-repo `gnx find --mode bm25` shape
