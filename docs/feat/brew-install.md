# Feature: `brew install cgn` (Homebrew tap)
...
Neither precondition is met today, so the README line _"`brew tap coseto6125/tap && brew install code-graph-nexus` — Available after the tap formula is published with the first Release"_ is a placeholder.
...
2. **Add `Formula/cgn.rb`** in that tap repo. Skeleton:

   ```ruby
   class Cgn < Formula
     desc "Code intelligence graph for AI agents and LLMs"
     homepage "https://github.com/coseto6125/code-graph-nexus"
     ...
     head "https://github.com/coseto6125/code-graph-nexus.git", branch: "main"

     depends_on "rust" => :build

     def install
       # Workspace has multiple bins — point cargo at the cli crate.
       system "cargo", "install", *std_cargo_args(path: "crates/cgn-cli")
     end

     test do
       assert_match "code-graph-nexus", shell_output("#{bin}/cgn --version")
       assert_equal 9, shell_output("#{bin}/cgn mcp tools").lines.count
     end
   end
   ```
...
4. **Test**: `brew install --build-from-source ./Formula/cgn.rb` locally, then `brew tap coseto6125/tap && brew install cgn` end-to-end.
5. **CI**: optionally add a release-time job that auto-updates `Formula/cgn.rb` in the tap repo with the new `url` + computed `sha256` on each tag push.

## Decision points to revisit

- **Stable vs HEAD-only**: HEAD-only formula doesn't require Releases at all — `brew install --HEAD cgn` does a source build. Cheaper to maintain but means brew users compile from scratch each `brew upgrade`.
- **Tap repo visibility**: must be public for Homebrew to clone. Aligns with going-public step.
- **Cask vs Formula**: cgn is a Rust CLI, Formula (build from source or fetch upstream binary) is correct. Cask would be wrong (that's for GUI .app bundles).

## Acceptance criteria

- `brew tap coseto6125/tap && brew install cgn` on a clean macOS box → working `cgn --version` + `cgn mcp tools` lists 9 tools.
