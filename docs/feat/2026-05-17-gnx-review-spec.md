...
Individual gnx primitives (`scan`, `impact`, `shape-check`, `coverage`,
...
Outcome today: code-edit → LLM ships → reviewer finds the issue gnx
...
```
gnx review                       # default: git diff HEAD
gnx review --since main          # since branch divergence
gnx review --files crates/foo/*  # explicit list
```
...
- Not a replacement for language-native linters (`cargo check`, `ruff`,
  `tsc`). Those run first; `gnx review` adds the graph-aware semantic
...
| `--repo <path>` | cwd | Standard gnx repo selector |
...
```
crates/graph-nexus-cli/src/commands/review/
...
- Should `gnx review` auto-trigger language-native linters too, or
  strictly stay in graph-semantic territory? Default for MVP: stay in
  gnx territory. Hook composition handles the linter side.
