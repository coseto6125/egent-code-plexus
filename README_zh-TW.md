...
**我們落後上游的地方**（多數 ⚠️）：**Constructor Inference** 是最大缺口 — 只有 Python 有完整的 receiver-type binding，其他 13 種都還是 partial。**Frameworks** 在 Kotlin / C# / Swift / C / C++ / Dart 沒接（上游有，我們 parser 在但沒接 framework helper）。**Entry Points** 14 種語言皆已透過跨語言 scorer 覆蓋（`main()` 偵測 + HTTP 路由 + framework decorator ≥ 0.8 信心值）。
