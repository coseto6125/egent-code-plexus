use clap::Subcommand;
use ecp_core::registry::{resolve_home_ecp, FileLock, GroupEntry, RegistryFile};
...
fn mutate_registry<F>(op: F) -> Result<(), EcpError>
where
    F: FnOnce(&mut RegistryFile) -> Result<bool, EcpError>,
{
    let ecp = resolve_home_ecp();
    let lock_path = ecp.join("registry.json.lock");
    let _lock = FileLock::acquire_exclusive(&lock_path)
        .map_err(|e| EcpError::InvalidArgument(format!("flock: {e}")))?;

    let registry_path = ecp.join("registry.json");
    ...
    if mutated {
        RegistryFile::write_atomic(&registry_path, &reg)
            .map_err(|e| EcpError::InvalidArgument(format!("registry write: {e}")))?;
    }
    Ok(())
}
            ...
            .ok_or_else(|| {
                EcpError::Output(format!(
                    "repo not found in registry: {repo}\n\
                     → register the repo first with `ecp admin index`"
                ))
            })?;
...
fn remove(repo: &str, group: &str) -> Result<(), EcpError> {
    mutate_registry(|reg| {
        let mut changed = false;
