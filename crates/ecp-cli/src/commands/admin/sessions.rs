//! `ecp admin sessions list` — inspect L1 sessions under all repos.
//! reset / sweep variants deferred (parent spec §11.2 follow-up).

use clap::{Args, Subcommand};
use ecp_core::registry::resolve_home_ecp;
use ecp_core::session::{SessionMeta, SessionState};
use std::fs;
use std::io;

#[derive(Subcommand, Debug)]
pub enum SessionsCommand {
    /// List active L1 sessions across all repos under ~/.ecp/
    List(ListArgs),
}
...
fn run_list(args: ListArgs) -> io::Result<()> {
    ...
    let rows = collect_rows(&home_ecp)?;
...
fn collect_rows(home_ecp: &std::path::Path) -> io::Result<Vec<ListRow>> {
    let mut out = vec![];
    if !home_ecp.exists() {
        return Ok(out);
    }
    for repo_entry in fs::read_dir(home_ecp)? {
        ...
        for s_entry in fs::read_dir(&sessions_dir)? {
            ...
            let state = match &sm {
                ...
                None => SessionState::Stale {
                    reason: ecp_core::session::StaleReason::MetaUnreadable,
                },
            };
