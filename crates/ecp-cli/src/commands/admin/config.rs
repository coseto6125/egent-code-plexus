//! `ecp config` — interactive TUI wizard for repo-local config.
//!
//! Layout (top → bottom):
//!   1. ANSI-Shadow title `ECP FOR LLM` (static, ECP cyan → gold gradient)
...
use ecp_core::config::{config_path, load, save, Config};
...
pub fn run(args: ConfigArgs) -> Result<(), EcpError> {
    let repo_root = args
        .repo
        .as_ref()
        .map(PathBuf::from)
    ...
    if !io::stdout().is_terminal() {
        eprintln!(
            "ecp config requires an interactive terminal.\n\
    ...
    let cfg = load(&repo_root).map_err(EcpError::InvalidArgument)?;
    let mut app = App::new(cfg, repo_root);
    let mut terminal = enter_tui().map_err(EcpError::Io)?;
    let outcome = run_loop(&mut terminal, &mut app);
    leave_tui(&mut terminal).ok();
    outcome.map_err(EcpError::Io)?;
...
const CYAN: Color = Color::Rgb(0, 180, 216); // ECP primary
...
fn render_title(frame: &mut ratatui::Frame<'_>, area: Rect) {
    ...
    for row in 0..6 {
        let spans = vec![
            Span::raw("  "),
            Span::styled(TITLE_ECP[row], Style::default().fg(CYAN)),
            Span::raw("  "),
...
/// Paint each char of a beaver-sprite row with ECP palette per role:
...
// and show the user what `ecp config` will look like without launching
// the full TUI loop.
// ─────────────────────────────────────────────────────────────────────────
...
mod snapshot_tests {
    ...
    ///     cargo test -p egent-code-plexus --lib config::snapshot_tests -- --nocapture
    #[test]
    fn print_snapshot() {
