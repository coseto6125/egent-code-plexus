//! `ecp admin drop` — delete a repo's on-disk index data AND registry entry.
//!
//! Targets the repo identified by `--repo` (path, defaulting to cwd).
//! Side effects:
//!   * delete `<home_ecp>/<repo.name>/` (index dir, all branches)
...
pub fn run(args: DropArgs) -> Result<(), ecp_core::EcpError> {
    let home_ecp = ecp_core::registry::resolve_home_ecp();
    let registry = ecp_core::registry::Registry::open(&home_ecp)
        .map_err(|e| ecp_core::EcpError::InvalidArgument(format!("registry: {e}")))?;

    if args.all {
        let snapshot = registry.snapshot().clone();
        for dir_name in snapshot.repos.keys() {
            let index_dir = home_ecp.join(dir_name);
            let _ = ecp_core::registry::retire_dir_async(&index_dir)?;
        }
        // Rewrite registry removing all entries under exclusive lock.
        drop(registry);
        rewrite_without(&home_ecp, None)?;

        if let Ok(audit) = ecp_core::registry::AuditLog::open(&home_ecp.join("audit.log")) {
            let _ = audit.append(&ecp_core::registry::AuditEvent::RegistryMutate {
                op: "drop-all".into(),
                repo: "all".into(),
                branch: None,
            });
        ...
        let dir_name = repo_dir_name_for_cwd(&args.repo)
            .map_err(|e| ecp_core::EcpError::InvalidArgument(format!("repo_identity: {e}")))?;
        ...
        drop(registry);
        rewrite_without(&home_ecp, Some(&dir_name))?;
...
fn rewrite_without(home_ecp: &Path, repo_name: Option<&str>) -> Result<(), ecp_core::EcpError> {
    ...
    let mut current = ecp_core::registry::RegistryFile::read_or_empty(&registry_path)
        .map_err(|e| ecp_core::EcpError::InvalidArgument(format!("read: {e}")))?;
    ...
    ecp_core::registry::RegistryFile::write_atomic(&registry_path, &current)
        .map_err(|e| ecp_core::EcpError::InvalidArgument(format!("write: {e}")))?;
    Ok(())
}
