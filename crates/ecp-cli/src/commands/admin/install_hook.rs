//! `ecp admin install-hook`: install reference-transaction hook in cwd's git common dir.
...
pub struct InstallHookArgs {
    /// Force overwrite if a non-ecp hook already exists.
    #[arg(long, default_value_t = false)]
    pub force: bool,

    /// Skip hook chaining (don't preserve existing non-ecp hook).
    #[arg(long, default_value_t = false)]
    pub no_chain: bool,
...
pub fn run(args: InstallHookArgs) -> Result<(), ecp_core::EcpError> {
    if args.claude_code {
    ...
    let out = safe_exec::git()
        ...
        .output()
        .map_err(|e| ecp_core::EcpError::InvalidArgument(format!("git: {e}")))?;
    if !out.status.success() {
        return Err(ecp_core::EcpError::InvalidArgument(
    ...
    let ecp_bin = std::env::current_exe()?;
    let ecp_bin_str = ecp_bin.to_string_lossy().into_owned();

    let existing_chain_target = if hook_path.exists() {
        let existing = std::fs::read_to_string(&hook_path).unwrap_or_default();
        if existing.contains("ecp hook-handle") || existing.contains("hook-handle") {
            None
    ...
    content.push_str(&format!("exec \"{ecp_bin_str}\" hook-handle \"$@\"\n"));
