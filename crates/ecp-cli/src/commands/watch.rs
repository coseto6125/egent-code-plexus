...
pub fn run(args: WatchArgs) -> Result<(), EcpError> {
    ...
    match (args.start, args.stop, args.status, args.foreground) {
        ...
        _ => Err(EcpError::InvalidArgument(
            "specify exactly one of --start | --stop | --status | --foreground".into(),
        )),
    }
}

fn start_foreground(repo_root: PathBuf, sid: String, session_dir: PathBuf) -> Result<(), EcpError> {
    if std::env::var("ECP_TEST_EXIT_AFTER_INIT").is_ok() {
        eprintln!("[ecp watch] test mode — exiting after init");
        return Ok(());
    }
    let cfg = WatcherCfg {
        repo_root,
    ...
    run_watcher(cfg).map_err(EcpError::Io)
}

#[cfg(unix)]
fn start_background(repo_root: PathBuf, sid: String, session_dir: PathBuf) -> Result<(), EcpError> {
    ...
    eprintln!("[ecp watch] forked watcher pid={pid}, sid={sid}");
    Ok(())
}

#[cfg(not(unix))]
fn start_background(_: PathBuf, _: String, _: PathBuf) -> Result<(), EcpError> {
    Err(EcpError::InvalidArgument(
