//! `ecp group sync <name>` — extract contracts from all group members,
...
pub fn run(args: SyncArgs) -> Result<(), EcpError> {
    ...
    //    as the alias string passed to `ecp admin group add`, which may be
...
fn extract_member(
    ...
    let alias = lookup_member(reg, member).ok_or_else(|| {
        (
            m.clone(),
            "not found in registry (run `ecp admin index <path>` first)".to_string(),
        )
    })?;
