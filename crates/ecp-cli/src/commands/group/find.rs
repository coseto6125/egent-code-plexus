//! `ecp group find <name> <pattern>` — BM25 symbol lookup across all group
...
pub enum MergeMode {
    ...
    /// header. Matches the single-repo `ecp find --mode bm25` shape.
    None,
...
pub fn run(args: FindArgs) -> Result<(), EcpError> {
    ...
    let engines: Vec<(String, Engine)> = resolve_member_engines(group, &reg, &home_ecp);
