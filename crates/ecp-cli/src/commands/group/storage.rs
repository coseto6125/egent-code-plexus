...
//! mirrors `ecp_core::registry::io::atomic_write_json`.
...
pub struct GroupMeta {
    ...
    /// `"default"` = built-in defaults; `"file"` = loaded from `~/.ecp/config.toml`.
...
pub fn write_contracts(group_dir: &Path, reg: &ContractRegistry) -> io::Result<()> {
    ...
    ecp_core::registry::atomic_write_bytes(&path, &bytes)
}
...
pub fn write_meta(group_dir: &Path, meta: &GroupMeta) -> io::Result<()> {
    fs::create_dir_all(group_dir)?;
    ecp_core::registry::atomic_write_json(&group_dir.join(META_FILE), meta)
}
