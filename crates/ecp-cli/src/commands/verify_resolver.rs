//! `ecp verify-resolver` — diff our resolver dump against a language oracle.
//!
//! Spec: `docs/specs/2026-05-15-resolver-oracle-harness.md`.
//!
//! Reads two JSONL files (oracle output + ecp `--dump-resolver` output),
...
pub struct VerifyResolverArgs {
    ...
    /// Path to the ecp resolver dump JSONL (produced by
    /// `ecp analyze --dump-resolver`).
    #[arg(long)]
    pub ecp: PathBuf,
...
pub fn run(args: VerifyResolverArgs) -> Result<(), ecp_core::EcpError> {
    let oracle = read_jsonl(&args.oracle)
        .map_err(|e| ecp_core::EcpError::InvalidArgument(format!("read oracle: {e}")))?;
    let ecp = read_jsonl(&args.ecp)
        .map_err(|e| ecp_core::EcpError::InvalidArgument(format!("read ecp dump: {e}")))?;

    let normalize = pick_normalize(&args.lang);
    let (counts, worst, per_tier) = diff(&oracle.records, &ecp.records, normalize);
            ...
            if let Some(parent) = p.parent() {
                if !parent.as_os_str().is_empty() {
                    std::fs::create_dir_all(parent).map_err(|e| {
                        ecp_core::EcpError::InvalidArgument(format!("mkdir report parent: {e}"))
                    })?;
                }
            }
            std::fs::write(p, &report)
                .map_err(|e| ecp_core::EcpError::InvalidArgument(format!("write report: {e}")))?;
...
fn diff(
    ...
    // Index ecp by (src_file, name) — there may be multiple ecp attempts per
    ...
    let mut ecp_by_key: FxHashMap<(String, String), &Record> = FxHashMap::default();
        ...
        match ecp_by_key.get(&key) {
            ...
            Some(g) => {
                ...
                match (o.target_file.as_deref(), g.target_file.as_deref()) {
                    (Some(ot), Some(gt)) => {
                        if normalize(ot) == normalize(gt) {
                            ...
                            push_offender(
                                ...
                                o,
                                "fp_ghost",
                                &format!("ecp→{} oracle→{}", normalize(gt), normalize(ot)),
                            );
                        }
                    }
                    (Some(_), None) => {
                    ...
                    (None, Some(_)) => {
                        // Oracle said unresolved but ecp connected → conservative FP_ghost
                        counts.fp_ghost += 1;
                        entry.fp_ghost += 1;
                    }
                    (None, None) => {
    ...
    // ecp_only: ecp decisions for keys oracle never produced (often Tier 1 same-file)
    for g in ecp {
        let key = (g.src_file.clone(), g.name.clone());
        if !oracle_keys.contains(&key) {
            if g.tier == "SameFile" {
                counts.ecp_only_same_file += 1;
            } else {
                counts.ecp_only_other += 1;
            }
        }
    }
...
fn render_report(
    ...
    s.push_str(&format!(
        "Oracle records: {oracle_total} (bad lines: {oracle_bad})\nEcp records: {ecp_total} (bad lines: {ecp_bad})\n\n"
    ));
    if oracle_bad > 0 || ecp_bad > 0 {
        s.push_str(
    ...
    s.push_str(&format!(
        "| FN_dangling (oracle resolved, ecp didn't) | {} |\n",
        counts.fn_dangling
    ));
    s.push_str(&format!(
    ...
    s.push_str(&format!(
        "| oracle_only (oracle imports ecp never saw) | {} |\n",
        counts.oracle_only
    ));
    s.push_str(&format!(
        "| ecp_only same-file (excluded from diff) | {} |\n",
        counts.ecp_only_same_file
    ));
    s.push_str(&format!(
        "| ecp_only other (no oracle counterpart) | {} |\n",
        counts.ecp_only_other
    ));
    s.push('\n');

    s.push_str("## Per-tier breakdown (ecp side)\n\n");
...
mod tests {
    ...
    fn diff_classifies_tp_when_target_files_match_under_normalization() {
        ...
        let (c, _, _) = diff(&oracle, &ecp, normalize_ts);
        assert_eq!(c.tp, 1);
    ...
    #[test]
    fn diff_classifies_fn_dangling_when_ecp_unresolved() {
        let oracle = vec![mk(
            "s.ts",
            "X",
            Some("@/y"),
        ...
        let ecp = vec![mk("s.ts", "X", Some("@/y"), "Unresolved", None, 0)];
        let (c, _, _) = diff(&oracle, &ecp, normalize_ts);
    ...
    #[test]
    fn diff_prefers_best_ecp_attempt_per_key() {
        let oracle = vec![mk(
            "s.ts",
            "X",
            Some("@/y"),
        ...
        // ecp records two attempts for same (src, name): Unresolved then Global → Global wins
        let ecp = vec![
    ...
    #[test]
    fn diff_counts_oracle_only_when_ecp_never_saw_the_key() {
        // Oracle has a binding ecp never resolved (e.g. import in a file
    ...
    #[test]
    fn diff_treats_oracle_unresolved_plus_ecp_connected_as_fp_ghost() {
        // Oracle says the import didn't resolve; ecp still produced an
        // edge (likely Tier-3 same-name match). Conservative: count as
        // ghost since ecp is connecting things tsc couldn't.
    ...
    fn diff_ignores_both_unresolved_pairs() {
        ...
        let ecp = vec![mk("s.ts", "X", Some("nope"), "Unresolved", None, 0)];
        let (c, _, _) = diff(&oracle, &ecp, normalize_ts);
        assert_eq!(c.tp, 0);
        assert_eq!(c.fp_ghost, 0);
        assert_eq!(c.fn_dangling, 0);
    ...
    #[test]
    fn diff_buckets_ecp_only_records_by_tier() {
        // ecp resolved Tier 1 SameFile entries oracle never sees — those
        // should land in `ecp_only_same_file` (excluded from diff). Other
        // tier hits oracle never sees go to `ecp_only_other`.
        ...
        let (c, _, _) = diff(&oracle, &ecp, normalize_ts);
        assert_eq!(c.ecp_only_same_file, 1);
        assert_eq!(c.ecp_only_other, 1);
    }
    ...
    fn read_jsonl_skips_malformed_lines_and_counts_them() {
        use std::io::Write;
        let path =
            std::env::temp_dir().join(format!("ecp-jsonl-test-{}.jsonl", std::process::id()));
