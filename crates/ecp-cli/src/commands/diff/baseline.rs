...
fn warn_if_local_diverges_from_remote(ref_str: &str, local_sha: &str, repo_dir: &Path) {
    ...
    eprintln!(
        "warning: local `{ref_str}` ({}) differs from `{remote_ref}` ({}).\n\
         ecp is using local. Sync with `git pull --ff-only origin {ref_str}`,\n\
...
fn resolve_via_git(ref_str: &str, repo_dir: &Path) -> Result<String, EcpError> {
    let out = safe_exec::git()
        .args(["rev-parse", "--verify", &format!("{ref_str}^{{commit}}")])
        .current_dir(repo_dir)
        .output()
        .map_err(|e| EcpError::Output(format!("git rev-parse failed to spawn: {e}")))?;
