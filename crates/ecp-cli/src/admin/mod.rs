//! `ecp admin` — interactive TUI for operational maintenance.
//!
//! Opens a `dialoguer`-based menu tree.  Index, agent integration,
//! config, group, and diagnostic workflows are reachable from here.
//! Scriptable host-integration operations live under `ecp admin <subcmd>`;
...
pub fn run(_args: AdminArgs) -> Result<(), EcpError> {
    let theme = ColorfulTheme::default();
    main_menu(&theme)
}

fn main_menu(theme: &ColorfulTheme) -> Result<(), EcpError> {
    loop {
        let choice = menu::select(theme, "ecp admin", MAIN_MENU)?;
        match choice {
...
pub const MAIN_MENU: &[menu::Item<'_>] = &[
    ...
    ("Config", "view, edit, validate ecp.toml"),
