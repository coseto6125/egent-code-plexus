//! Claude Code MCP integration — called from `ecp admin claude` via
...
pub(crate) fn install_scripted() -> Result<(), EcpError> {
    ...
    let output = Command::new("claude")
        .args(args)
        .output()
        .map_err(|e| EcpError::Output(format!("spawn claude: {e}")))?;
...
fn command_error(command: &str, output: &std::process::Output) -> EcpError {
    let stderr = String::from_utf8_lossy(&output.stderr).trim().to_string();
    let stdout = String::from_utf8_lossy(&output.stdout).trim().to_string();
    let detail = if stderr.is_empty() { stdout } else { stderr };
    EcpError::Output(format!("{command}: {detail}"))
}
...
mod tests {
    ...
    fn install_args_register_user_scoped_stdio_server() {
        let args = install_args("/usr/local/bin/ecp");
        ...
        assert_eq!(spec["command"], "/usr/local/bin/ecp");
