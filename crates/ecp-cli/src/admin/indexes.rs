//! Index maintenance workflows for `ecp admin`.
...
fn build_refresh_wizard(theme: &ColorfulTheme) -> Result<(), EcpError> {
    ...
    .map_err(EcpError::Output)
}

fn inspect_indexed_repos() -> Result<(), EcpError> {
    ...
    print_registry(registry.snapshot(), &home_ecp);
    Ok(())
}

fn print_registry(registry: &RegistryFile, home_ecp: &std::path::Path) {
    println!("Registry: {}", home_ecp.join("registry.json").display());
...
fn prune_wizard(theme: &ColorfulTheme) -> Result<(), EcpError> {
...
fn drop_wizard(theme: &ColorfulTheme) -> Result<(), EcpError> {
    ...
    if all {
        let confirmed = Confirm::with_theme(theme)
            .with_prompt("This deletes all ecp index data and registry entries. Continue")
            .default(false)
            .interact()
            .map_err(dialoguer_err)?;
        if confirmed {
    ...
    let confirmed = Confirm::with_theme(theme)
        .with_prompt(format!("Drop ecp index data for {}", repo.display()))
        .default(false)
        .interact()
        .map_err(dialoguer_err)?;
    if confirmed {
...
fn input(theme: &ColorfulTheme, prompt: &str, default: &str) -> Result<String, EcpError> {
...
fn input_path(theme: &ColorfulTheme, prompt: &str, default: &str) -> Result<PathBuf, EcpError> {
    input(theme, prompt, default).map(PathBuf::from)
}

fn dialoguer_err(e: dialoguer::Error) -> EcpError {
    EcpError::Output(format!("dialoguer: {e}"))
}
...
mod tests {
    ...
    fn print_registry_accepts_empty_and_populated_registry() {
        print_registry(&RegistryFile::empty(), std::path::Path::new("/tmp/ecp"));
        ...
        print_registry(&registry, std::path::Path::new("/tmp/ecp"));
    }
}
