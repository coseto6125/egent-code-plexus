//! Group management workflows for `ecp admin`.
...
fn add_repo_wizard(theme: &ColorfulTheme) -> Result<(), EcpError> {
...
fn remove_repo_wizard(theme: &ColorfulTheme) -> Result<(), EcpError> {
    let repo = input(theme, "Repo name")?;
    let group_name = input(theme, "Group name")?;
...
fn sync_contracts() {
    ...
    match Registry::open(&home_ecp) {
        Ok(registry) => {
            ...
            println!("Contract sync is not implemented yet; use `ecp contracts` for inventory.");
        }
        Err(e) => eprintln!("Cannot open registry: {e}"),
    }
}

fn input(theme: &ColorfulTheme, prompt: &str) -> Result<String, EcpError> {
