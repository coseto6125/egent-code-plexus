ecp index: {{stats.nodes}} symbols, {{stats.edges}} rels at HEAD {{head}}.
Use `ecp inspect <name>` for symbol context, `ecp find <name>` for
exact-name definition lookup (add `--mode fuzzy` for substring or
`--mode bm25` for BM25 ranking), `ecp impact <name>` for blast radius.
{{#if graphify}}
