//! Auto-watch spawn: session_start checks for <repo>/.ecp/auto-watch marker.
...
fn marker_present_session_start_still_returns_quickly() {
    let dir = tempdir().unwrap();
    let repo = dir.path();
    std::fs::create_dir_all(repo.join(".ecp")).unwrap();
    std::fs::write(repo.join(".ecp/auto-watch"), "").unwrap();
