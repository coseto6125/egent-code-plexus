...
//! on this shape; ecp must produce zero).
...
// ecp rule (spec §2: "resolver miss → don't emit"): when a TS file imports
// `./foo`, ecp must only link to foo.ts in the same directory, never to
