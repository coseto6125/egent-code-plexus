...
fn group_find_unknown_group_exits_nonzero() {
    ...
    let out = run_ecp(&["group", "find", "__no_such_group__", "foo"], tmp.path());
    assert!(
        !out.status.success(),
...
fn group_find_json_shape_two_repos() {
    ...
    for dn in &dir_names {
        let out = run_ecp(&["admin", "group", "add", dn, "findgrp"], home);
        assert!(
            out.status.success(),
    ...
    let out = run_ecp(&["group", "find", "findgrp", "hello", "--json"], home);
    assert!(
        out.status.success(),
        "group find failed:\nstdout: {}\nstderr: {}",
...
// ── --merge rrf (consolidated from former `ecp group search`) ─────────────────
...
fn group_find_merge_rrf_json_shape_two_repos() {
    ...
    let dir_names = read_dir_names(&home_ecp);
    for dn in &dir_names {
        let out = run_ecp(&["admin", "group", "add", dn, "rrfgrp"], home);
        assert!(
            out.status.success(),
            "admin group add failed for {dn}:\nstderr: {}",
            String::from_utf8_lossy(&out.stderr)
