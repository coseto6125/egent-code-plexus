...
//! `crates/ecp-cli/src/config_parser.rs`, but lives as an integration
//! test so the public `parse_single_*` entry points stay exercised through
//! the `ecp_cli` crate surface.

use ecp_cli::config_parser::{parse_single_composer_json, parse_single_swift_package};
use std::fs;
