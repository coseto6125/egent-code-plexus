//! Tests `ecp admin index` writes to ~/.ecp/<repo>/<branch>/ and updates registry.
//! (replaces the old `ecp analyze` top-level command, folded into admin)
...
fn analyze_writes_to_registry_resolved_path() {
    ...
    let entries: Vec<_> = walkdir::WalkDir::new(&ecp_root)
        .max_depth(4)
        .into_iter()
    ...
    // Per-repo RepoMeta (v2): ~/.ecp/<repo>__<hash>/meta.json
