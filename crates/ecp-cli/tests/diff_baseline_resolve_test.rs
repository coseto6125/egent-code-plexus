//! Verify `ecp diff` CLI surface: required args, section enum, baseline rejection.
...
fn diff_baseline_pr_form_calls_gh() {
    ...
    // Use a clearly non-existent PR; ecp should surface a clean error.
    let output = Command::new(env!("CARGO_BIN_EXE_ecp"))
...
fn diff_baseline_short_name_warns_on_remote_divergence() {
    ...
    // Run ecp diff with --baseline main; expect warning on stderr.
    let output = Command::new(env!("CARGO_BIN_EXE_ecp"))
...
fn diff_baseline_short_name_no_remote_emits_note() {
    ...
    // Run ecp diff --baseline main. No origin remote → expect skip-note on stderr.
    let output = Command::new(env!("CARGO_BIN_EXE_ecp"))
        .args(["diff", "--section", "bindings", "--baseline", "main"])
        .current_dir(repo)
        .env("HOME", repo)
