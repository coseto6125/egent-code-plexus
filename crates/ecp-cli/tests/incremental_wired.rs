...
// ── L1 freshness gate (FU-2026-06-10-f92cecba3daa) ──────────────────────────

/// A repeat query over an UNCHANGED dirty working tree must not re-parse the
/// dirty set: fragments record each parsed file's mtime_ns in
/// `dirty_files.json`, so the Stale arm can prove the overlay is already
/// current with stat calls alone. Observable contract (counters are
/// process-local, the work happens in a spawned `ecp`): the `l1.refreshed
/// written=N` stderr line appears only when fragments were actually written.
#[test]
fn test_fresh_fragments_skip_reanalyze_on_repeat_query() {
    let tmp = TempDir::new().unwrap();
    let worktree = tmp.path().join("repo");
    let home = tmp.path().join("home");
    fs::create_dir_all(&worktree).unwrap();
    ...
    // Same session id across queries — the manifest lives under the sid dir.
    let find = || {
    ...
    // Make both files dirty (mtime strictly newer than graph.bin).
    std::thread::sleep(Duration::from_millis(50));
    fs::write(
        worktree.join("lib.rs"),
        "pub fn original_fn() {}\npub fn added_a() {}\n",
    ...
    let s1 = find();
    assert!(
        s1.contains("l1.refreshed written=2"),
        "first query must refresh both dirty files; stderr={s1}"
    );

    let s2 = find();
    assert!(
        !s2.contains("l1.refreshed"),
        "repeat query with unchanged dirty files must skip the L1 refresh entirely; stderr={s2}"
    );

    // Touch ONE file — only that file may be re-parsed.
    std::thread::sleep(Duration::from_millis(50));
    fs::write(
        worktree.join("util.rs"),
        "pub fn helper_fn() {}\npub fn added_b2() {}\n",
