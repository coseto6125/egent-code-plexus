//! Verify `ecp shape-check --route <path>` filters Fetches edges
...
/// initialize git, run `ecp admin index` with isolated HOME so the registry
...
fn shape_check_help_lists_route_arg() {
    ...
    let output = Command::new(ecp_bin())
        ...
        .output()
        .expect("run ecp shape-check --help");
...
fn shape_check_route_no_match_emits_helpful_message() {
    ...
    let output = Command::new(ecp_bin())
        ...
        .env("HOME", repo)
        .output()
        .expect("run ecp shape-check --route");
