//! Integration tests for `ecp group status`.

use ecp_cli::commands::group::storage::{group_dir, write_meta, GroupMeta, RepoSnapshot};
...
fn read_first_dir_name(home_ecp: &Path) -> String {
...
fn status_never_synced_reports_no_meta() {
    ...
    let dir_name = read_first_dir_name(&home_ecp);

    // admin group add <repo> <group>
    let out = run_ecp(&["admin", "group", "add", &dir_name, "demo"], home);
    assert!(
        out.status.success(),
    ...
    let out = run_ecp(&["group", "status", "demo"], home);
    assert!(
        out.status.success(),
        "group status failed:\nstdout: {}\nstderr: {}",
        String::from_utf8_lossy(&out.stdout),
