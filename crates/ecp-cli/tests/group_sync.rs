//! Integration test for `ecp group sync`.
//!
...
use ecp_cli::commands::group::storage::{group_dir, read_contracts};
use std::path::Path;
...
"#,
    ...
    let registry_path = home.join(".ecp").join("registry.json");
    let reg = ecp_core::registry::RegistryFile::read_or_empty(&registry_path).unwrap();
    ...
    let out = run_ecp(&["admin", "group", "add", &go_dir_name, "demo"], home);
    assert!(
        out.status.success(),
    ...
    let out = run_ecp(&["admin", "group", "add", &py_dir_name, "demo"], home);
    assert!(
        out.status.success(),
    ...
    // ── 6. Run group sync ─────────────────────────────────────────────────
    let out = run_ecp(&["group", "sync", "demo"], home);
    assert!(
        out.status.success(),
        "group sync failed:\nstdout: {}\nstderr: {}",
        String::from_utf8_lossy(&out.stdout),
    ...
    let meta: ecp_cli::commands::group::storage::GroupMeta =
