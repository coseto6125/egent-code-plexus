use ecp_cli::commands::group::storage::{
...
fn meta_json_roundtrip() {
    ...
    ecp_cli::commands::group::storage::write_meta(dir.path(), &meta).unwrap();
    let read = ecp_cli::commands::group::storage::read_meta(dir.path()).unwrap();
