...
pub fn resolve(ref_str: &str, repo_dir: &Path) -> Result<String, CgnError> {
...
fn resolve_via_git(ref_str: &str, repo_dir: &Path) -> Result<String, CgnError> {
...
fn resolve_pr(pr_num: &str, repo_dir: &Path) -> Result<String, CgnError> {
    ...
    let out = Command::new("gh")
        ...
        .output()
        .map_err(|_| {
            CgnError::Output("gh CLI not found; install gh or pass commit SHA directly".into())
        })?;
