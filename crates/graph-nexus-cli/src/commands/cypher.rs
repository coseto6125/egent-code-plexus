...
static CYPHER_REGEX: LazyLock<Regex> = LazyLock::new(|| {
    Regex::new(
        r"(?i)MATCH\s+\((?P<src_var>\w+):(?P<src_kind>\w+)\)\s*<*-\[\s*(?P<rel_var>\w*):(?P<rel_type>\w+)\s*\]->*\s*\((?P<tgt_var>\w+):(?P<tgt_kind>\w+)\)\s*(?:WHERE\s+(?P<where_var>\w+)\.(?P<where_prop>\w+)\s*=\s*'(?P<where_val>[^']+)')?\s*RETURN\s+(?P<ret>[\w\.,\s]+)"
