...
//! `ecp_peers` tool fronting all sub-subcommands via `subcmd` discriminator.
...
// ── minimal synthetic CLI tree (no ecp binary needed) ────────────────────────
...
async fn single_ecp_peers_tool_registered() {
    ...
    assert!(
        names.contains(&"ecp_peers"),
        "missing ecp_peers; got {names:?}"
    );
    for stale in ["ecp_peers_status", "ecp_peers_log", "ecp_peers_say"] {
        assert!(
...
#[tokio::test(flavor = "current_thread")]
async fn ecp_peers_advertises_subcmd_discriminator() {
    let server = EcpMcpServer::new(&Cli::command()).expect("init");
    let tool = server
        .list_tools()
        .iter()
...
fn peers_tool() -> ecp_mcp::schema::DerivedTool {
    ecp_mcp::peers::peer_tools()
        .into_iter()
        .find(|t| t.name == "ecp_peers")
        .expect("ecp_peers tool")
}
