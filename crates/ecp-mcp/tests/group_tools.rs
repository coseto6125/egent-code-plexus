//! Smoke tests: `ecp_group` tool registration + spawn-argv shape for the
//! single tool fronting all 7 `ecp group <verb>` sub-subcommands via the
...
// ── minimal synthetic CLI tree mirroring `ecp group` being `hide = true` ────
...
async fn single_ecp_group_tool_registered() {
    ...
    let names: Vec<&str> = server
        ...
        .collect();
    assert!(
        names.contains(&"ecp_group"),
        "missing ecp_group; got {names:?}"
    );
    // The `hide = true` group subcommand must not produce its own
    // derived tool (the manual injection is the only entry).
    let group_count = names.iter().filter(|n| n.starts_with("ecp_group")).count();
    assert_eq!(
        group_count, 1,
        "expected exactly one ecp_group* tool; got {names:?}"
    );
}

#[tokio::test(flavor = "current_thread")]
async fn ecp_group_advertises_all_subcmds() {
    let server = EcpMcpServer::new(&Cli::command()).expect("init");
    let tool = server
        .list_tools()
        .iter()
...
fn group_tool() -> ecp_mcp::schema::DerivedTool {
    ecp_mcp::group::group_tools()
        .into_iter()
        .find(|t| t.name == "ecp_group")
        .expect("ecp_group tool")
}
