...
pub struct DerivedTool {
    ...
    /// sub-subcommand dispatch (e.g. `["status"]` → `ecp peers status`).
...
fn derive_tool(cmd: &Command) -> DerivedTool {
    ...
    let description = cmd
        .get_about()
        .map(|s| s.to_string())
        .unwrap_or_else(|| format!("ecp {subcommand}"));
