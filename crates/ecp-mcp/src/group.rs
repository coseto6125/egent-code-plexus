//! Manually-constructed MCP tool for `ecp group` sub-subcommands.
//!
//! The root `ecp group` subcommand is `#[command(hide = true)]` in main.rs,
//! so `enumerate_tools` skips it. Without this hand-rolled tool, LLM clients
//! would have no path to invoke `ecp group <verb>` via MCP — and the
...
//! `ecp group <subcmd> <name> [<pattern>] [--flags...]`.
...
/// Return the single `ecp_group` MCP tool fronting all group sub-subcommands.
...
fn tool_group() -> DerivedTool {
    DerivedTool {
        ...
        description: "Multi-repo group operations: extract contracts, query \
            ...
            Groups are managed via `ecp admin group add/remove`."
            .into(),
        schema: Arc::new(json!({
            "type": "object",
            "properties": {
                ...
                "name": {
                    "type": "string",
                    "description": "[all] Group name (must exist in registry; add members via `ecp admin group add <repo> <group>`)."
                },
                "pattern": {
                    "type": "string",
