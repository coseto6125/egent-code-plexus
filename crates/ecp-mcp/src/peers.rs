//! Manually-constructed MCP tool for `ecp peers` sub-subcommands.
//!
//! `enumerate_tools` would surface `ecp peers` as a single opaque tool with
...
fn tool_peers() -> DerivedTool {
    DerivedTool {
        ...
        description: "[multi-agent peer-sync only — single-session has no peers] \
            Inspect / talk to other ecp watch sessions on this repo. \
