...
fn parse_h(src: &str) -> LocalGraph {
    let provider = CProvider::new().expect("provider");
    provider.parse_file(Path::new("test.h"), src.as_bytes()).expect("parse")
}
...
fn test_c_proto_simple_emits_function() {
    ...
    let node = graph.nodes.iter().find(|n| n.name == "dictCreate" && n.kind == NodeKind::Function);
    assert!(node.is_some(), "expected Function node `dictCreate`, got {:#?}", graph.nodes);
}

#[test]
fn test_c_proto_void_return_emits_function() {
    let graph = parse_h("void sha256_init(SHA256_CTX *ctx);\n");
    let node = graph.nodes.iter().find(|n| n.name == "sha256_init" && n.kind == NodeKind::Function);
    assert!(node.is_some(), "expected Function node `sha256_init`, got {:#?}", graph.nodes);
}

#[test]
fn test_c_proto_size_t_return_emits_function() {
    let graph = parse_h("size_t hdr_get_memory_size(struct hdr_histogram *h);\n");
    let node = graph.nodes.iter().find(|n| n.name == "hdr_get_memory_size" && n.kind == NodeKind::Function);
    assert!(node.is_some(), "expected Function node `hdr_get_memory_size`, got {:#?}", graph.nodes);
}

#[test]
fn test_c_proto_pointer_return_emits_function() {
    let graph = parse_h("char *lpSeek(unsigned char *lp, long index);\n");
    let node = graph.nodes.iter().find(|n| n.name == "lpSeek" && n.kind == NodeKind::Function);
    assert!(node.is_some(), "expected Function node `lpSeek`, got {:#?}", graph.nodes);
}
...
fn test_c_forward_decl_in_c_file_emits_function() {
    ...
    let node = graph.nodes.iter().find(|n| n.name == "verifyClusterNodeId" && n.kind == NodeKind::Function);
    assert!(node.is_some(), "expected Function node `verifyClusterNodeId`, got {:#?}", graph.nodes);
}

#[test]
fn test_c_forward_decl_void_emits_function() {
    let graph = parse("void rdbLoadProgressCallback(rio *r, const void *buf, size_t len);\n");
    let node = graph.nodes.iter().find(|n| n.name == "rdbLoadProgressCallback" && n.kind == NodeKind::Function);
    assert!(node.is_some(), "expected Function node `rdbLoadProgressCallback`, got {:#?}", graph.nodes);
}
...
fn test_c_extern_func_proto_emits_function() {
    ...
    let node = graph.nodes.iter().find(|n| n.name == "sz_boot" && n.kind == NodeKind::Function);
    assert!(node.is_some(), "expected Function node `sz_boot`, got {:#?}", graph.nodes);
}
...
fn test_c_plain_union_emits_struct_node() {
    ...
    let node = graph.nodes.iter().find(|n| n.name == "bio_job" && n.kind == NodeKind::Struct);
    assert!(node.is_some(), "expected Struct node for union `bio_job`, got {:#?}", graph.nodes);
}
...
fn test_c_typedef_union_emits_struct_and_typedef() {
    ...
    let struct_node = graph.nodes.iter().find(|n| n.name == "typeData" && n.kind == NodeKind::Struct);
    let typedef_node = graph.nodes.iter().find(|n| n.name == "typeData" && n.kind == NodeKind::Typedef);
    assert!(struct_node.is_some(), "expected Struct node for union `typeData`, got {:#?}", graph.nodes);
    assert!(typedef_node.is_some(), "expected Typedef node `typeData`, got {:#?}", graph.nodes);
}
...
fn test_c_anonymous_union_no_crash() {
    ...
    let typedef_node = graph.nodes.iter().find(|n| n.name == "MyUnion" && n.kind == NodeKind::Typedef);
    assert!(typedef_node.is_some(), "expected Typedef node `MyUnion`, got {:#?}", graph.nodes);
}
...
fn test_c_function_definition_still_captured() {
    ...
    let node = graph.nodes.iter().find(|n| n.name == "add" && n.kind == NodeKind::Function);
    assert!(node.is_some(), "expected Function node `add`, got {:#?}", graph.nodes);
}
...
fn test_c_local_variable_not_emitted() {
    ...
    let local = graph.nodes.iter().find(|n| n.name == "i" && n.kind == NodeKind::Variable);
    assert!(local.is_none(), "local var `i` must not be emitted, got {:#?}", graph.nodes);
}
...
fn test_c_static_function_not_exported() {
    ...
    let node = graph.nodes.iter().find(|n| n.name == "helper").expect("helper must be emitted");
