//! TypeScript identifier finder. Covers:
//! - `identifier` (function names, variables, parameters, imported bindings)
//! - `type_identifier` (interface / type alias / generic params)
//! - `property_identifier` (member access, object literal keys, method defs)
...
pub fn find_identifier_occurrences_tsx(source: &[u8], target_name: &str) -> Vec<IdentifierRange> {
    find_by_kinds(
        source,
        target_name,
...
mod tests {
    ...
    #[test]
    fn finds_type_alias_and_use() {
        let src = b"type Foo = number;\nlet x: Foo = 1;\n";
        let hits = find_identifier_occurrences(src, "Foo");
