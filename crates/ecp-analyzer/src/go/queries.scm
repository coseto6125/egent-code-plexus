...
;; File-scope `const X = value` (single) and `const ( A = 1; B = 2 )` (grouped).
...
;; `Const`; without this query ecp dropped 42 top-level Go consts on
...
(call_expression
  ...
  arguments: (argument_list
    [(interpreted_string_literal) @route.path (MISSING) @route.path]
