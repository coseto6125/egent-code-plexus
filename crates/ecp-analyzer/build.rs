//! Build script for `ecp-analyzer`.
//!
//! Computes a SHA256 fingerprint of all parser-related source files at
//! compile time and emits it as the `ECP_PARSER_FINGERPRINT`
...
fn main() {
    ...
    if std::env::var_os("ECP_BUILD_VERBOSE").is_some() {
    ...
    println!("cargo:rustc-env=ECP_PARSER_FINGERPRINT={hex}");
    println!("cargo:rerun-if-env-changed=ECP_BUILD_VERBOSE");
    // Opt-in via `ECP_BUILD_VERBOSE=1`; env var above keeps the fingerprint
    // available to runtime code regardless.
    if std::env::var_os("ECP_BUILD_VERBOSE").is_some() {
        println!("cargo:warning=ecp-analyzer parser fingerprint: {hex}");
    }
}
