...
fn python_app_test_client_chained_calls_emit_zero() {
    ...
    // running ecp with `--include-tests` on sanic-org/sanic. A user who
...
fn python_direct_client_get_emits_route_documented_limitation() {
    ...
    // test files which get filtered by default in `ecp routes`.
    let src = r#"
...
fn python_flask_blueprint_transitive_import_extracts() {
    // Real-world Flask pattern that ecp previously missed (recall gap
...
fn php_laravel_nested_middleware_group_routes_extract() {
    // Real-world Laravel pattern that ecp previously missed (PR #50
