...
//! guarded class headers, so ecp stopped emitting those classes
...
//! binding. The assertions verify the `(kind, name)` tuples ecp
...
//! the file doubles as documentation for what ecp considers a
...
def pi\n  3.14\nend\n";
    ...
    let g = p
        ...
        .expect("parse");
    // Ruby `def` is always a Method in ecp's taxonomy — Ruby has no
