...
fn view_fn_is_exported() {
    let g = parse("@view\ndef view_fn() -> uint256:\n    return 0\n");
    assert_eq!(find_exported(&g, "view_fn"), Some(true), "`view_fn` must be exported");
}
...
fn external_view_fn_is_exported() {
    let g = parse("@external\n@view\ndef both():\n    pass\n");
    assert_eq!(find_exported(&g, "both"), Some(true), "`both` (external+view) must be exported");
}

#[test]
fn bare_fn_is_not_exported() {
    let g = parse("def plain():\n    pass\n");
    assert_eq!(find_exported(&g, "plain"), Some(false), "bare `plain` must NOT be exported");
}
