...
//! N concurrent `ecp` invocations.

use ecp_core::registry::{Registry, RepoAlias};
use std::path::PathBuf;

fn main() {
    let mut args = std::env::args().skip(1);
    let home_ecp = PathBuf::from(args.next().expect("arg 1: home_ecp path"));
    let repo_name = args.next().expect("arg 2: repo name");
    let marker = args.next().expect("arg 3: slot marker");

    let mut reg = Registry::open(&home_ecp).expect("registry open");
