...
fn accepts_valid_names() {
    assert_eq!(
        sanitize_segment("egent-code-plexus").unwrap(),
        "egent-code-plexus"
    );
...
use ecp_core::registry::derive_repo_name;

#[test]
fn derives_from_ssh_url() {
    let r = derive_repo_name(Some("git@github.com:coseto6125/egent-code-plexus.git")).unwrap();
    assert_eq!(r, "egent-code-plexus");
}

#[test]
fn derives_from_https_url() {
    let r = derive_repo_name(Some("https://github.com/coseto6125/egent-code-plexus.git")).unwrap();
    assert_eq!(r, "egent-code-plexus");
}
...
fn uid_strips_repo_prefix() {
    let abs = Path::new("/home/enor/egent-code-plexus/src/auth.ts");
    let repo = Path::new("/home/enor/egent-code-plexus");
