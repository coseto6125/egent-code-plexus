...
//!
//! Real production failure mode: multiple `ecp` invocations from Claude
...
use ecp_core::registry::Registry;
...
fn registry_concurrent_writers_converge() {
    ...
    let home_ecp = tmp.path().to_path_buf();
    ...
    let reg = Registry::open(&home_ecp).expect("open final");
